...
{
	/*
	�lad macierzy - tylko dla kwadratowej bo og�lna wersja nie jest potrzebna
	*/
	double tr=0;
	for(int i=1;i<=a[1][0];i++)
		tr=tr+a[i][i];
...
{
	/*
	Funkcja oblicza �redni� elementow wektora
	*/
	double s=0;
	for(int i=1;i<=a[0];i++)
		s+=a[i];
...
{
	/*
	Funkcja oblicza �redni� wartosci bezwzglednych elementow wektora
	*/
	double s=0;
	for(int i=1;i<=a[0];i++)
		s+=fabs(a[i]);
...
{
	/*
	Funkcja liczy iloczyn wektorowy dw�ch trzyelementowych wektor�w
	*/
	c[1]=a[2]*b[3]-b[2]*a[3];
	c[2]=a[3]*b[1]-b[3]*a[1];
	c[3]=a[1]*b[2]-b[1]*a[2];
