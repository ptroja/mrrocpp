...
Dane sa uzupelniane w razie potrzeby zerami, tak aby n' bylo wielokrotnoscia w. Okna nie zachodza na siebie*/
	...
	if (rest>0) {
		...
		newdata = new double[n];  // Uwaga: tu jest alokacja pamiêci, jej zwolnienie musi byæ zapewnione po stronie wywo³ania (WK)
	}
...
/* Dokonuje redukcji rozdzielczoœci (polowy+1) widma spektrogramu danego tablic¹ 
  ...
  Metoda usredniania z nachodzeniem na siebie usrednianych blokow. */
{
	...
	// Czêsci ca³kowite poni¿szych dzieleñ.
