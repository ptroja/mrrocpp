...
//Constructors
mm_test::mm_test(lib::configurator &_config) :
...
void mm_test::mp_2_ecp_next_state_string_handler(void)
{
	...
	if (mp_2_ecp_next_state_string == ecp_mp::generator::ECP_GEN_NEWSMOOTH) {
		...
		path += (char*) mp_command.ecp_next_state.sg_buf.data; //mp_2_ecp_next_state_string;

		if (((char*) mp_command.ecp_next_state.sg_buf.data)[0] <= '9'
				&& ((char*) mp_command.ecp_next_state.sg_buf.data)[0] >= '0') {
			double t[4];
			lib::setValuesInArray(t, (char*) mp_command.ecp_next_state.sg_buf.data);
		...
		double mp_args[2];
		lib::setValuesInArray(mp_args, (char*) mp_command.ecp_next_state.sg_buf.data);
