		...
		common::task::task(_config)
{
	...
	if (config.robot_name == lib::irp6ot_m::ROBOT_NAME) {
        ...
        register_sg(new subtask::subtask_const_vel_gen_test(*this));

        register_sg(new subtask::subtask_smooth_gen_test(*this));

        register_sg(new subtask::subtask_spline_gen_test(*this));
