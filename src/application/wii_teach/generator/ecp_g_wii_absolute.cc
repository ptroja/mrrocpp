...
wii_absolute::wii_absolute(common::task::task& _ecp_task,
...
void wii_absolute::set_position(void) {
	...
	rotation[0][1] = cos(nextChange[6]) * sin(nextChange[4]) * sin(
			nextChange[5]) - sin(nextChange[6]) * cos(nextChange[5]);
	rotation[1][1] = sin(nextChange[6]) * sin(nextChange[4]) * sin(
			nextChange[5]) + cos(nextChange[6]) * cos(nextChange[5]);
	rotation[2][1] = cos(nextChange[4]) * sin(nextChange[5]);
	rotation[0][2] = cos(nextChange[6]) * sin(nextChange[4]) * cos(
			nextChange[5]) + sin(nextChange[6]) * sin(nextChange[5]);
	rotation[1][2] = sin(nextChange[6]) * sin(nextChange[4]) * cos(
			nextChange[5]) - cos(nextChange[6]) * sin(nextChange[5]);
