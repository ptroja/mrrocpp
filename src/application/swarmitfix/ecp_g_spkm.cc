...
bool spkm_pose::first_step()
{
	...
	if(the_robot->nextstateBuffer->Get().segments.empty())
		return false;

	// set iterator to the first command
	segment_iterator = the_robot->nextstateBuffer->Get().segments.begin();
...
bool spkm_pose::next_step()
{
	...
	if (segment_iterator == the_robot->nextstateBuffer->Get().segments.end())
		return false;
