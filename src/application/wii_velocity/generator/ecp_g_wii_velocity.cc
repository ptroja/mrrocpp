...
bool wii_velocity::next_step()
{
	...
	if (operate) {
		...
		configure_velocity(wii->image.up && !wii->image.right ? wii->image.orientation_y * C_0 : 0, wii->image.right
				&& !wii->image.up && !wii->image.down ? wii->image.orientation_y * C_1 : 0, wii->image.down
				&& !wii->image.right ? wii->image.orientation_y * C_2 : 0, wii->image.left ? wii->image.orientation_y
				* C_3 : 0, wii->image.up && wii->image.right ? wii->image.orientation_y * C_4 : 0, wii->image.down
				&& wii->image.right ? wii->image.orientation_y * C_5 : 0);
	} else {
