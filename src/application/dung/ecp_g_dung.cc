...
bool dung::next_step()
{
	...
	q3 = the_robot->reply_package.arm.pf_def.arm_coordinates[2]
			- the_robot->reply_package.arm.pf_def.arm_coordinates[1];
	if (oq3 >= 100) {
		dq3 = 0.0;
		oq3 = q3;
	} else {
	...
	q4 = the_robot->reply_package.arm.pf_def.arm_coordinates[3]
			- the_robot->reply_package.arm.pf_def.arm_coordinates[2] - M_PI_2;
	if (oq4 >= 100) {
		dq4 = 0.0;
		oq4 = q4;
	} else {
	...
	g[0] = 0.0;
	g[1] = 9.8 * (17.57 * cos(q2) - 0.343 * cos(q2 + q3) - 1.05 * sin(q2 + q3 + q4) + 0.05 * cos(q2 + q3 + q4)
			* sin(q5));
