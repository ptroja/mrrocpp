...
void visualservo_tester::main_task_algorithm(void)
{
	...
	if (run_conveyor) {
		...
		set_next_ecps_state(mrrocpp::ecp_mp::common::generator::ECP_GEN_CONVEYOR_VS_TEST, 0, "", 0, 1, lib::conveyor::ROBOT_CONVEYOR.c_str());

		sr_ecp_msg->message("Conveyor started.");
	}

	run_extended_empty_generator_for_set_of_robots_and_wait_for_task_termination_message_of_another_set_of_robots(2, 2, ROBOT_NAME.c_str(), lib::conveyor::ROBOT_CONVEYOR.c_str(), ROBOT_NAME.c_str(), lib::conveyor::ROBOT_CONVEYOR.c_str());
