...
extern "C"
{
	void on_arrow_button_clicked_</xsl:text><xsl:value-of select="$fullName" /><xsl:text>_axis_ts (GtkButton* button, gpointer userdata)
	{
		...
		</xsl:text><xsl:call-template name="irp6.axis.ts.repeat.signals.cc.arrow">
	...
	</xsl:text><xsl:call-template name="irp6.axis.ts.repeat.signals.cc.read.1">
    		<xsl:with-param name="axis_ts" select="$axis_ts"/>
			<xsl:with-param name="i" select="1"/>
			<xsl:with-param name="name" select="$name"/>
 		</xsl:call-template><xsl:text>
...
</xsl:text><xsl:call-template name="irp6.axis.ts.repeat.signals.cc.read.2">
    				<xsl:with-param name="axis_ts" select="$axis_ts"/>
					<xsl:with-param name="name" select="$name"/>
					<xsl:with-param name="i" select="1"/>
 				</xsl:call-template><xsl:text>				
	...
	void on_set_button_clicked_</xsl:text><xsl:value-of select="$fullName" /><xsl:text>_axis_ts (GtkButton* button, gpointer userdata)
	...
	</xsl:text><xsl:call-template name="irp6.axis.ts.repeat.signals.cc.set.1">
    		<xsl:with-param name="axis_ts" select="$axis_ts"/>
			<xsl:with-param name="i" select="1"/>
			<xsl:with-param name="name" select="$name"/>
 		</xsl:call-template><xsl:text>    
		...
		{
	</xsl:text><xsl:call-template name="irp6.axis.ts.repeat.signals.cc.set.2">
    		<xsl:with-param name="axis_ts" select="$axis_ts"/>
			<xsl:with-param name="name" select="$name"/>
			<xsl:with-param name="i" select="1"/>
 		</xsl:call-template><xsl:text>    
...
<xsl:template name="irp6.axis.ts.repeat.signals.cc.set.1">
...
<xsl:param name="name"/>
	...
	<!-- for loop --> 
       <xsl:if test="$i &lt;= $axis_ts">
          <xsl:call-template name="irp6.axis.ts.repeat.signals.cc.set.1">
...
<!-- irp6 servo algorithm repeatable part -->
<xsl:template name="irp6.axis.ts.repeat.signals.cc.read.1">
<xsl:param name="axis_ts"/>
<xsl:param name="i"/>
<xsl:param name="name"/>
	<xsl:if test="$i &lt;= $axis_ts">
	...
	<!-- for loop --> 
       <xsl:if test="$i &lt;= $axis_ts">
          <xsl:call-template name="irp6.axis.ts.repeat.signals.cc.read.1">
              <xsl:with-param name="i">
                  <xsl:value-of select="$i + 1"/>
              </xsl:with-param>
              <xsl:with-param name="axis_ts">
...
<!-- irp6 servo algorithm repeatable part -->
<xsl:template name="irp6.axis.ts.repeat.signals.cc.read.2">
<xsl:param name="axis_ts"/>
<xsl:param name="name"/>
<xsl:param name="i"/>
	<xsl:if test="$i &lt;= $axis_ts">
...
<xsl:text>					snprintf (buf, sizeof(buf), "%.3f", tool_vector_a[</xsl:text><xsl:value-of select="($i - 1)" /><xsl:text>]);
	...
	<!-- for loop --> 
       <xsl:if test="$i &lt;= $axis_ts">
          <xsl:call-template name="irp6.axis.ts.repeat.signals.cc.read.2">
              <xsl:with-param name="i">
                  <xsl:value-of select="$i + 1"/>
              </xsl:with-param>
              <xsl:with-param name="axis_ts">
...
<!-- irp6 servo algorithm repeatable part -->
<xsl:template name="irp6.axis.ts.repeat.signals.cc.set.2">
<xsl:param name="axis_ts"/>
<xsl:param name="name"/>
<xsl:param name="i"/>
	<xsl:if test="$i &lt;= $axis_ts">
	...
	<!-- for loop --> 
       <xsl:if test="$i &lt;= $axis_ts">
          <xsl:call-template name="irp6.axis.ts.repeat.signals.cc.set.2">
              <xsl:with-param name="i">
                  <xsl:value-of select="$i + 1"/>
              </xsl:with-param>
              <xsl:with-param name="axis_ts">
...
<xsl:template name="irp6.axis.ts.repeat.signals.cc.arrow">
<xsl:param name="axis_ts"/>
<xsl:param name="i"/>
<xsl:param name="name"/>
	<xsl:if test="$i &lt;= $axis_ts">
	...
	<!-- for loop --> 
       <xsl:if test="$i &lt;= $axis_ts">
          <xsl:call-template name="irp6.axis.ts.repeat.signals.cc.arrow">
              <xsl:with-param name="i">
                  <xsl:value-of select="$i + 1"/>
              </xsl:with-param>
              <xsl:with-param name="axis_ts">
