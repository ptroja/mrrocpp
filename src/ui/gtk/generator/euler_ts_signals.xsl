...
extern "C"
{
	...
	void on_read_button_clicked_</xsl:text><xsl:value-of select="$fullName" /><xsl:text>_euler_ts (GtkButton* button, gpointer userdata)
	{
		...
		if (robot_</xsl:text><xsl:value-of select="$fullName" /><xsl:text></xsl:text><xsl:choose><xsl:when test="$name = 'conveyor'"></xsl:when><xsl:otherwise><xsl:text>->ecp</xsl:text></xsl:otherwise></xsl:choose><xsl:text>->get_EDP_pid()!=-1)
		{
			...
			{
				if (!( robot_</xsl:text><xsl:value-of select="$fullName" /><xsl:text>->read_tool_xyz_euler_zyz(tool_vector))) // Odczyt polozenia walow silnikow
	...
	void on_set_button_clicked_</xsl:text><xsl:value-of select="$fullName" /><xsl:text>_euler_ts (GtkButton* button, gpointer userdata)
	{
		...
		if (state.is_synchronised)
		{
			...
			robot_</xsl:text><xsl:value-of select="$fullName" /><xsl:text>->set_tool_xyz_euler_zyz(tool_vector);
		}
