...
MpPanel::MpPanel(ui_config_entry &entry)
	...
	struct builder_widget {
		...
		const char *name;
			...
			{ &MpStartPauseButton, "base/mpStartPauseButton" },
			{ &MpStopButton, "base/mpStopButton" },
			{ &MpTriggerButton, "base/mpTriggerButton" },
	...
	for (size_t i = 0; i < sizeof(builder_widgets)/sizeof(builder_widgets[0]); i++) {
		*builder_widgets[i].object = GTK_BUTTON(gtk_builder_get_object(&builder, builder_widgets[i].name));
		if(!*builder_widgets[i].object) {
			fprintf(stderr, "base/mp button object %d (\"%s\") assignment failed\n", i, builder_widgets[i].name);
...
void MpPanel::StartButtonMode(StartButtonMode_t mode) {
	...
	GtkImage *button_image = GTK_IMAGE(gtk_builder_get_object(&builder, "base/mpStartButtonImage"));
	...
	GtkLabel *button_label = GTK_LABEL(gtk_builder_get_object(&builder, "base/mpStartButtonLabel"));
