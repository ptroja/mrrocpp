...

{
	...
	try { // dla bledow robot :: ECP_error
		...
		if (ui_state.irp6p_tfg.edp.state == 0) {
			...
			if ((!(ui_state.irp6p_tfg.edp.test_mode)) && (access(
...

				{
					...
					ui_robot.irp6p_tfg = new ui_tfg_and_conv_robot(*ui.config,
							*ui.all_ecp_msg, lib::ROBOT_IRP6P_TFG);
				}

				ui_state.irp6p_tfg.edp.pid
						= ui_robot.irp6p_tfg->ecp->get_EDP_pid();
				...
				if (ui_state.irp6p_tfg.edp.pid < 0) {
					...
					while ((ui_state.irp6p_tfg.edp.reader_fd
							= name_open(
									ui_state.irp6p_tfg.edp.network_reader_attach_point.c_str(),
...

{
	...
	try {
		...
		if ((ui_state.irp6p_tfg.edp.state > 0)
			...
			ui_state.irp6p_tfg.edp.is_synchronised
					= ui_robot.irp6p_tfg->ecp->is_synchronised();
		} else {
	...
	if (ui_state.irp6p_tfg.edp.pid != -1) {
		...
		if ((((ApName(ApWidget(cbinfo))
				...
				|| ((cbinfo->event->type == Ph_EV_KEY) && (my_data->key_cap
						== 0x73))) && (ui_state.irp6p_tfg.edp.is_synchronised)) {// powrot do pozycji synchronizacji
			...
			for (int i = 0; i < IRP6P_TFG_NUM_OF_SERVOS; i++) {
				irp6p_tfg_desired_pos[i]
						= ui_state.irp6p_tfg.edp.preset_position[0][i];
			}
			edp_irp6p_tfg_eb.command(
			...
			for (int i = 0; i < IRP6P_TFG_NUM_OF_SERVOS; i++) {
				irp6p_tfg_desired_pos[i]
						= ui_state.irp6p_tfg.edp.preset_position[1][i];
			}
			edp_irp6p_tfg_eb.command(
					boost::bind(irp6p_tfg_execute_joint_motion));
		} else if ((((ApName(ApWidget(cbinfo))
			...
			for (int i = 0; i < IRP6P_TFG_NUM_OF_SERVOS; i++) {
				irp6p_tfg_desired_pos[i]
						= ui_state.irp6p_tfg.edp.preset_position[2][i];
			}
			edp_irp6p_tfg_eb.command(
					boost::bind(irp6p_tfg_execute_joint_motion));
		}
...

{
	...
	try {
		if (ui_state.irp6p_tfg.edp.pid != -1) {
			...
			{
				ui_robot.irp6p_tfg->get_servo_algorithm(servo_alg_no,
						servo_par_no);
...

{
	...
	try {
		if (ui_state.irp6p_tfg.edp.is_synchronised) {
			...
			ui_robot.irp6p_tfg->set_servo_algorithm(servo_alg_no_output,
					servo_par_no_output);
...

{
	...
	try {
		if ((ui_state.irp6p_tfg.edp.pid != -1)
				&& (ui_state.is_wind_irp6p_tfg_moves_open)) {
			if (ui_state.irp6p_tfg.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
			...
			for (int i = 0; i < 3; i++) {
				...
				if (ui.config->exists(tmp_string,
					...
					for (int j = 0; j < IRP6P_TFG_NUM_OF_SERVOS; j++) {
						...
						ui_state.irp6p_tfg.edp.preset_position[i][j] = strtod(
								tmp1, &tmp1);
