	...
	try {
		if (ui.irp6ot_m->state.edp.pid != -1) {
			if (ui.irp6ot_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				...
				ui.irp6ot_m->ui_ecp_robot->read_motors(ui.irp6ot_m->irp6ot_current_pos); // Odczyt polozenia walow silnikow

				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p0, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p1, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p2, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p3, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p4, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p5, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[5], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p6, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[6], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_motors_cur_p7, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[7], 0);
				/*
...
int wnd_irp6ot_motors_copy_current_to_desired(PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo)
{
	...
	// wychwytania ew. bledow ECP::robot
	double *wektor_ptgr[lib::irp6ot_m::NUM_OF_SERVOS], wektor[lib::irp6ot_m::NUM_OF_SERVOS];
...

{
	...
	if (ui.irp6ot_m->state.edp.pid != -1) {
		...
		if ((((ApName(ApWidget(cbinfo)) == ABN_mm_irp6_on_track_preset_position_synchro) || (ApName(ApWidget(cbinfo))
				...
				&& (my_data->key_cap == 0x73))) && (ui.irp6ot_m->state.edp.is_synchronised)) {// powrot do pozycji synchronizacji
			for (int i = 0; i < lib::irp6ot_m::NUM_OF_SERVOS; i++) {
			...
			for (int i = 0; i < lib::irp6ot_m::NUM_OF_SERVOS; i++) {
				ui.irp6ot_m->irp6ot_desired_pos[i] = ui.irp6ot_m->state.edp.preset_position[0][i];
			}
			...
			for (int i = 0; i < lib::irp6ot_m::NUM_OF_SERVOS; i++) {
				ui.irp6ot_m->irp6ot_desired_pos[i] = ui.irp6ot_m->state.edp.preset_position[1][i];
			}
			ui.irp6ot_m->eb.command(boost::bind(irp6ot_execute_joint_motion));
			...
			for (int i = 0; i < lib::irp6ot_m::NUM_OF_SERVOS; i++) {
				ui.irp6ot_m->irp6ot_desired_pos[i] = ui.irp6ot_m->state.edp.preset_position[2][i];
			}
			ui.irp6ot_m->eb.command(boost::bind(irp6ot_execute_joint_motion));
				...
				== Ph_EV_KEY) && (my_data->key_cap == 0x66))) && (ui.irp6ot_m->state.edp.is_synchronised)) {// ruch do pozycji zadania (wspolrzedne przyjete arbitralnie)
			for (int i = 0; i < lib::irp6ot_m::NUM_OF_SERVOS; i++) {
				ui.irp6ot_m->irp6ot_desired_pos[i] = ui.irp6ot_m->state.edp.front_position[i];
			}
			ui.irp6ot_m->eb.command(boost::bind(irp6ot_execute_joint_motion));
		}
...

{
	...
	double *wektor[lib::irp6ot_m::NUM_OF_SERVOS];
	double *step1;
	...
	try {
		...
		if (ui.irp6ot_m->state.edp.pid != -1) {
			...
			if ((ui.irp6ot_m->state.edp.is_synchronised) && (ui.irp6ot_m->is_wind_irp6ot_inc_open)) { // by Y o dziwo niedziala poprawnie 	 if (ui.irp6ot_m->state.edp.is_synchronised)

				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p0, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p1, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p2, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p3, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p4, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p5, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[5], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p6, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[6], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_inc_p7, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[7], 0);
...
int init_wnd_irp6_on_track_int(PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo)
{
	...
	try {
		if (ui.irp6ot_m->state.edp.pid != -1) {
			if (ui.irp6ot_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				ui.irp6ot_m->ui_ecp_robot->read_joints(ui.irp6ot_m->irp6ot_current_pos); // Odczyt polozenia walow silnikow

				// 	ui.unblock_widget(ABW_PtPane_wind_irp6ot_int_post_synchro_moves);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p1, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p2, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p3, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p4, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p5, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p6, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[5], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p7, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[6], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_joints_cur_p8, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[7], 0);
...
int irp6ot_int_motion(PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo)
{
	...
	// wychwytania ew. bledow ECP::robot
	try {
		if ((ui.irp6ot_m->state.edp.pid != -1) && (ui.irp6ot_m->state.edp.is_synchronised)) {
			...
			if (ui.irp6ot_m->is_wind_irp6ot_int_open) // Czy robot jest zsynchronizowany?
			{
				...
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p1, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p2, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p3, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p4, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p5, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p6, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[5], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p7, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[6], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_int_p8, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[7], 0);
			}
...
int init_wnd_irp6_on_track_xyz_euler_zyz(PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo)
{
	...
	try {
		if (ui.irp6ot_m->state.edp.pid != -1) {
			if (ui.irp6ot_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				ui.irp6ot_m->ui_ecp_robot->read_xyz_euler_zyz(ui.irp6ot_m->irp6ot_current_pos); // Odczyt polozenia walow silnikow

				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_read_p1, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_read_p2, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_read_p3, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_read_p4, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_read_p5, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_read_p6, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[5], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_read_p7, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[6], 0);
...

{
	...
	try {
		if (ui.irp6ot_m->state.edp.is_synchronised) {
			...
			ui.irp6ot_m->ui_ecp_robot->move_xyz_euler_zyz(ui.irp6ot_m->irp6ot_desired_pos);

			PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_p1, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[0], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_p2, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[1], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_p3, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[2], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_p4, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[3], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_p5, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[4], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_p6, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[5], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_p7, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_desired_pos[6], 0);
		}
	} // end try
	CATCH_SECTION_UI
...
int init_wnd_irp6_on_track_xyz_angle_axis(PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo)
{
	...
	try {
		if (ui.irp6ot_m->state.edp.pid != -1) {
			if (ui.irp6ot_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				ui.irp6ot_m->ui_ecp_robot->read_xyz_angle_axis(ui.irp6ot_m->irp6ot_current_pos); // Odczyt polozenia walow silnikow

				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_angle_axis_read_p1, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_angle_axis_read_p2, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_angle_axis_read_p3, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_angle_axis_read_p4, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_angle_axis_read_p5, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_angle_axis_read_p6, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[5], 0);

				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_angle_axis_read_p8, Pt_ARG_NUMERIC_VALUE, &ui.irp6ot_m->irp6ot_current_pos[6], 0);
...

{
	...
	try {
		if (ui.irp6ot_m->state.edp.is_synchronised) {
			...
			ui.irp6ot_m->ui_ecp_robot->move_xyz_angle_axis(ui.irp6ot_m->irp6ot_desired_pos);
...

{
	...
	ui.irp6ot_m->eb.command(boost::bind(EDP_irp6_on_track_synchronise_int, widget, apinfo, cbinfo));
...

{
	...
	try {
		...
		if ((ui.irp6ot_m->state.edp.state > 0) && (ui.irp6ot_m->state.edp.is_synchronised == false)) {
			ui.irp6ot_m->ui_ecp_robot->ecp->synchronise();
			ui.irp6ot_m->state.edp.is_synchronised = ui.irp6ot_m->ui_ecp_robot->ecp->is_synchronised();
		} else {
	...
	try {
		if (ui.irp6ot_m->state.edp.pid != -1) {
			if (ui.irp6ot_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				...
				double w;
				w = tool_vector[0];
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p1, Pt_ARG_NUMERIC_VALUE, &w, 0);
				w = tool_vector[1];
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p2, Pt_ARG_NUMERIC_VALUE, &w, 0);
				w = tool_vector[2];
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p3, Pt_ARG_NUMERIC_VALUE, &w, 0);
				w = tool_vector[3];
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p4, Pt_ARG_NUMERIC_VALUE, &w, 0);
				w = tool_vector[4];
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p5, Pt_ARG_NUMERIC_VALUE, &w, 0);
				w = tool_vector[5];
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p6, Pt_ARG_NUMERIC_VALUE, &w, 0);
...

{
	...
	uint8_t servo_alg_no[lib::irp6ot_m::NUM_OF_SERVOS];
	uint8_t servo_par_no[lib::irp6ot_m::NUM_OF_SERVOS];
	...
	try {
		if (ui.irp6ot_m->state.edp.pid != -1) {
			if (ui.irp6ot_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				ui.irp6ot_m->ui_ecp_robot->get_servo_algorithm(servo_alg_no, servo_par_no); // Odczyt polozenia walow silnikow
...

{
	...
	// wychwytania ew. bledow ECP::robot
	uint8_t *wektor_ptgr[lib::irp6ot_m::NUM_OF_SERVOS], *wektor2_ptgr[lib::irp6ot_m::NUM_OF_SERVOS],
			wektor[lib::irp6ot_m::NUM_OF_SERVOS], wektor2[lib::irp6ot_m::NUM_OF_SERVOS];
...

{
	...
	uint8_t *servo_alg_no_tmp[lib::irp6ot_m::NUM_OF_SERVOS];
	uint8_t servo_alg_no_output[lib::irp6ot_m::NUM_OF_SERVOS];
	uint8_t *servo_par_no_tmp[lib::irp6ot_m::NUM_OF_SERVOS];
	uint8_t servo_par_no_output[lib::irp6ot_m::NUM_OF_SERVOS];
	...
	try {
		if (ui.irp6ot_m->state.edp.is_synchronised) {
			...
			// zlecenie wykonania ruchu
			ui.irp6ot_m->ui_ecp_robot->set_servo_algorithm(servo_alg_no_output, servo_par_no_output);
...

{
	...
	if (ui.irp6ot_m->state.edp.state == 0) {
		...
		ui.irp6ot_m->eb.command(boost::bind(EDP_irp6_on_track_create_int, widget, apinfo, cbinfo));
	}
	return (Pt_CONTINUE);
...

{
	...
	try { // dla bledow robot :: ECP_error
		...
		if (ui.irp6ot_m->state.edp.state == 0) {
			...
			tmp2_string += ui.irp6ot_m->state.edp.network_resourceman_attach_point;

			// sprawdzenie czy nie jest juz zarejestrowany zarzadca zasobow
			if (((!(ui.irp6ot_m->state.edp.test_mode)) && (access(tmp_string.c_str(), R_OK) == 0))
					|| (access(tmp2_string.c_str(), R_OK) == 0)) {
				ui.ui_msg->message(lib::NON_FATAL_ERROR, "edp_irp6_on_track already exists");
			} else if (ui.check_node_existence(ui.irp6ot_m->state.edp.node_name, std::string("edp_irp6_on_track"))) {

				ui.irp6ot_m->state.edp.node_nr = ui.config->return_node_number(ui.irp6ot_m->state.edp.node_name);

				{
					boost::unique_lock < boost::mutex > lock(ui.process_creation_mtx);

					ui.irp6ot_m->ui_ecp_robot
							= new uin::common::ui_irp6_common_robot(*ui.config, *ui.all_ecp_msg, lib::irp6ot_m::ROBOT_NAME);
				}
				...
				if (ui.irp6ot_m->state.edp.pid < 0) {
					...
					ui.irp6ot_m->ui_ecp_robot->get_controller_state(robot_controller_initial_state_tmp);

					//ui.irp6ot_m->state.edp.state = 1; // edp wlaczone reader czeka na start

					ui.irp6ot_m->state.edp.is_synchronised = robot_controller_initial_state_tmp.is_synchronised;
				}
			}
		}
...

{
	...
	if (ui.irp6ot_m->state.edp.is_synchronised) { // o ile ECP dziala (sprawdzanie poprzez dzialanie odpowiedniego EDP)
		...
		if (ui.irp6ot_m->state.ecp.trigger_fd >= 0) {
			if (MsgSendPulse(ui.irp6ot_m->state.ecp.trigger_fd, sched_get_priority_min(SCHED_FIFO), pulse_code, pulse_value)
					== -1) {
...

{
	...
	try {
		if (ui.irp6ot_m->state.edp.is_synchronised) {
			...
			// zlecenie wykonania ruchu
			ui.irp6ot_m->ui_ecp_robot->move_xyz_angle_axis_relative(ui.irp6ot_m->irp6ot_desired_pos);
...

{
	...
	sprintf(buffer, "edp_IRP6_OT XYZ_ANGLE_AXIS POSITION\n %f %f %f %f %f %f %f", *wektor[0], *wektor[1], *wektor[2], *wektor[3], *wektor[4], *wektor[5], *wektor[6]);
