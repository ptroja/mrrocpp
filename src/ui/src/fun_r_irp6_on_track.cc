	...
	try {
		if (ui_state.irp6_on_track.edp.pid != -1) {
			if (ui_state.irp6_on_track.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				...
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p1, Pt_ARG_NUMERIC_VALUE, &(tool_vector.w[0]) , 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p2, Pt_ARG_NUMERIC_VALUE, &(tool_vector.w[1]) , 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p3, Pt_ARG_NUMERIC_VALUE, &(tool_vector.w[2]) , 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p4, Pt_ARG_NUMERIC_VALUE, &(tool_vector.w[3]) , 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p5, Pt_ARG_NUMERIC_VALUE, &(tool_vector.w[4]) , 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6ot_xyz_euler_zyz_ts_read_p6, Pt_ARG_NUMERIC_VALUE, &(tool_vector.w[5]) , 0);
...

{
	...
	uint8_t servo_alg_no[IRP6_ON_TRACK_NUM_OF_SERVOS];
	uint8_t servo_par_no[IRP6_ON_TRACK_NUM_OF_SERVOS];
...

{
	...
	uint8_t *servo_alg_no_tmp[IRP6_ON_TRACK_NUM_OF_SERVOS];
	uint8_t servo_alg_no_output[IRP6_ON_TRACK_NUM_OF_SERVOS];
	uint8_t *servo_par_no_tmp[IRP6_ON_TRACK_NUM_OF_SERVOS];
	uint8_t servo_par_no_output[IRP6_ON_TRACK_NUM_OF_SERVOS];
	...
	try { // dla bledow robot :: ECP_error
		...
		if (ui_state.irp6_on_track.edp.state == 0) {
			...
			if ((!(ui_state.irp6_on_track.edp.test_mode)) && (access(tmp_string.c_str(), R_OK) == 0)
				...
				ui_robot.irp6_on_track = new ui_common_robot(*config, *ui_msg.all_ecp, lib::ROBOT_IRP6_ON_TRACK);
...

{
	...
	if (ui_state.irp6_on_track.edp.is_synchronised) { // o ile ECP dziala (sprawdzanie poprzez dzialanie odpowiedniego EDP)
