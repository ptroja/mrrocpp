...

{
	...
	try {
		if (ui_state.irp6_mechatronika.edp.pid != -1) {
			...
			{
				if (!(ui_robot.irp6_mechatronika->read_tool_xyz_euler_zyz(
...

{
	...
	try {
		if (ui_state.irp6_mechatronika.edp.pid != -1) {
			if (ui_state.irp6_mechatronika.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				if (!(ui_robot.irp6_mechatronika->get_servo_algorithm(
...
/* eliminate 'unreferenced' warnings */
{
	...
	try {
		if (ui_state.irp6_mechatronika.edp.pid != -1) {
			if (ui_state.irp6_mechatronika.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				if (!(ui_robot.irp6_mechatronika->read_xyz_angle_axis(
...

{
	...
	try {
		if (ui_state.irp6_mechatronika.edp.pid != -1) {
			if (ui_state.irp6_mechatronika.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				if (!(ui_robot.irp6_mechatronika->read_tool_xyz_angle_axis(
						tool_vector))) // Odczyt polozenia walow silnikow
					printf("Blad w read external\n");
...

{
	...
	try {
		if (ui_state.irp6_mechatronika.edp.pid != -1) {
			if (ui_state.irp6_mechatronika.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				if (!(ui_robot.irp6_mechatronika->read_xyz_euler_zyz(
						irp6m_current_pos))) // Odczyt polozenia walow silnikow
					printf("Blad w read external\n");
