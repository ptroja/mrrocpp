	...
	try { // dla bledow robot :: ECP_error
		...
		if (ui_state.irp6ot_tfg.edp.state == 0) {
			...
			if ((!(ui_state.irp6ot_tfg.edp.test_mode)) && (access(
				...
				ui_state.irp6ot_tfg.edp.node_nr = config->return_node_number(
						ui_state.irp6ot_tfg.edp.node_name);

				{
					boost::unique_lock<boost::mutex> lock(process_creation_mtx);
					ui_robot.irp6ot_tfg = new ui_tfg_and_conv_robot(*config,
				...
				if (ui_state.irp6ot_tfg.edp.pid < 0) {
					...
					while ((ui_state.irp6ot_tfg.edp.reader_fd
							= name_open(
									ui_state.irp6ot_tfg.edp.network_reader_attach_point.c_str(),
					...
					ui_state.irp6ot_tfg.edp.is_synchronised
	...
	if (ui_state.irp6ot_tfg.edp.pid != -1) {
		...
		if ((((ApName(ApWidget(cbinfo))
				...
				|| ((cbinfo->event->type == Ph_EV_KEY) && (my_data->key_cap
						== 0x73))) && (ui_state.irp6ot_tfg.edp.is_synchronised)) {// powrot do pozycji synchronizacji
			...
			for (int i = 0; i < IRP6OT_TFG_NUM_OF_SERVOS; i++) {
				irp6ot_tfg_desired_pos[i]
						= ui_state.irp6ot_tfg.edp.preset_position[0][i];
			}
			edp_irp6ot_tfg_eb.command(boost::bind(
			...
			for (int i = 0; i < IRP6OT_TFG_NUM_OF_SERVOS; i++) {
				irp6ot_tfg_desired_pos[i]
						= ui_state.irp6ot_tfg.edp.preset_position[1][i];
			}
			edp_irp6ot_tfg_eb.command(boost::bind(
					irp6ot_tfg_execute_joint_motion));
		} else if ((((ApName(ApWidget(cbinfo))
			...
			for (int i = 0; i < IRP6OT_TFG_NUM_OF_SERVOS; i++) {
				irp6ot_tfg_desired_pos[i]
						= ui_state.irp6ot_tfg.edp.preset_position[2][i];
			}
			edp_irp6ot_tfg_eb.command(boost::bind(
					irp6ot_tfg_execute_joint_motion));
		}
...

{
	...
	try {
		if ((ui_state.irp6ot_tfg.edp.pid != -1)
				&& (ui_state.is_wind_irp6ot_tfg_moves_open)) {
			if (ui_state.irp6ot_tfg.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
...
int reload_irp6ot_tfg_configuration() {
	...
	if ((ui_state.irp6ot_tfg.is_active = config->value<int> (
		...
		if (ui_state.is_mp_and_ecps_active) {
			ui_state.irp6ot_tfg.ecp.network_trigger_attach_point
					= config->return_attach_point_name(
							...
							ui_state.irp6ot_tfg.ecp.section_name);
		...
		case 0:
			...
			for (int i = 0; i < 3; i++) {
				...
				if (config->exists(tmp_string,
						ui_state.irp6ot_tfg.edp.section_name)) {
					char* tmp, *tmp1;
					tmp1 = tmp = strdup(config->value<std::string> (tmp_string,
							ui_state.irp6ot_tfg.edp.section_name).c_str());
					char* toDel = tmp;
					for (int j = 0; j < IRP6OT_TFG_NUM_OF_SERVOS; j++) {

						ui_state.irp6ot_tfg.edp.preset_position[i][j] = strtod(
								tmp1, &tmp1);
			...
			if (config->exists("test_mode",
					ui_state.irp6ot_tfg.edp.section_name))
				ui_state.irp6ot_tfg.edp.test_mode = config->value<int> (
						"test_mode", ui_state.irp6ot_tfg.edp.section_name);
			else
				ui_state.irp6ot_tfg.edp.test_mode = 0;

			ui_state.irp6ot_tfg.edp.hardware_busy_attach_point = config->value<
			...
			ui_state.irp6ot_tfg.edp.node_name = config->value<std::string> (
					"node_name", ui_state.irp6ot_tfg.edp.section_name);
			break;
		case 1:
		case 2:
