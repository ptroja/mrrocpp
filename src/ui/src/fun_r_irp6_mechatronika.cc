...

{
	...
	// wychwytania ew. bledow ECP::robot
	double *wektor_ptgr[interface.irp6m_m->number_of_servos], wektor[interface.irp6m_m->number_of_servos];
	...
	try {
		if (interface.irp6m_m->state.edp.pid != -1) {
			...
			if (interface.irp6m_m->state.edp.is_synchronised) {
				...
				for (int i = 0; i < interface.irp6m_m->number_of_servos; i++) {
					interface.irp6m_m->desired_pos[i] = 0.0;
				}
			}
			...
			if ((interface.irp6m_m->state.edp.is_synchronised) && (interface.irp6m_m->is_wind_irp6m_inc_open)) {
				...
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_inc_p0, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_inc_p1, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_inc_p2, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_inc_p3, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_inc_p4, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[4], 0);
	...
	try {
		if (interface.irp6m_m->state.edp.pid != -1) {
			if (interface.irp6m_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				...
				interface.irp6m_m->ui_ecp_robot->read_motors(interface.irp6m_m->current_pos); // Odczyt polozenia walow silnikow

				PtSetResource(ABW_PtNumericFloat_wind_irp6m_motors_cur_p0, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_motors_cur_p1, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_motors_cur_p2, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_motors_cur_p3, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_motors_cur_p4, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[4], 0);
...

{
	...
	double *wektor[interface.irp6m_m->number_of_servos];
	double *step1;
...

{
	...
	// wychwytania ew. bledow ECP::robot
	uint8_t *wektor_ptgr[interface.irp6m_m->number_of_servos], *wektor2_ptgr[interface.irp6m_m->number_of_servos],
			wektor[interface.irp6m_m->number_of_servos], wektor2[interface.irp6m_m->number_of_servos];
...

{
	...
	uint8_t servo_alg_no[interface.irp6m_m->number_of_servos];
	uint8_t servo_par_no[interface.irp6m_m->number_of_servos];
...

{
	...
	uint8_t *servo_alg_no_tmp[interface.irp6m_m->number_of_servos];
	uint8_t servo_alg_no_output[interface.irp6m_m->number_of_servos];
	uint8_t *servo_par_no_tmp[interface.irp6m_m->number_of_servos];
	uint8_t servo_par_no_output[interface.irp6m_m->number_of_servos];
...

{
	...
	try {
		if (interface.irp6m_m->state.edp.is_synchronised) {
			...
			// zlecenie wykonania ruchu
			interface.irp6m_m->ui_ecp_robot->move_xyz_angle_axis(interface.irp6m_m->desired_pos);
...
/* eliminate 'unreferenced' warnings */
{
	...
	try {
		if (interface.irp6m_m->state.edp.pid != -1) {
			if (interface.irp6m_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				interface.irp6m_m->ui_ecp_robot->read_xyz_angle_axis(interface.irp6m_m->current_pos); // Odczyt polozenia walow silnikow
				printf("Blad w read read_xyz_angle_axis\n");

				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_angle_axis_read_p1, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_angle_axis_read_p2, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_angle_axis_read_p3, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_angle_axis_read_p4, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_angle_axis_read_p5, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_angle_axis_read_p6, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[5], 0);
	...
	try {
		...
		if (interface.irp6m_m->state.edp.is_synchronised) {
			...
			if (ApName(ApWidget(cbinfo)) == ABN_PtButton_wind_irp6m_xyz_euler_zyz_6l) {
				// 	PtGetResource(ABW_PtNumericFloat_internal_step,Pt_ARG_NUMERIC_VALUE, &step1, 0 );
				interface.irp6m_m->desired_pos[5] -= (*step1);
			} else
			...
			interface.irp6m_m->ui_ecp_robot->move_xyz_euler_zyz(interface.irp6m_m->desired_pos);

			PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_p1, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[0], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_p2, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[1], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_p3, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[2], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_p4, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[3], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_p5, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[4], 0);
			PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_p6, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->desired_pos[5], 0);
...

{
	...
	try {
		if (interface.irp6m_m->state.edp.pid != -1) {
			if (interface.irp6m_m->state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				interface.irp6m_m->ui_ecp_robot->read_xyz_euler_zyz(interface.irp6m_m->current_pos); // Odczyt polozenia walow silnikow

				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_read_p1, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[0], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_read_p2, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[1], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_read_p3, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[2], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_read_p4, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[3], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_read_p5, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[4], 0);
				PtSetResource(ABW_PtNumericFloat_wind_irp6m_xyz_euler_zyz_read_p6, Pt_ARG_NUMERIC_VALUE, &interface.irp6m_m->current_pos[5], 0);
