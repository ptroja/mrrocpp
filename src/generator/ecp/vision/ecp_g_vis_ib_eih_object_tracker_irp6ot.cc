...
bool ecp_vis_ib_eih_object_tracker_irp6ot::first_step()
{
	...
	//printf("first step\n");
	flushall();
	vsp_fradia = dynamic_cast <fradia_sensor_tracker *> (sensor_m[ecp_mp::sensor::SENSOR_FRADIA]);
...
bool ecp_vis_ib_eih_object_tracker_irp6ot::next_step_without_constraints()
{
	...
	the_robot->ecp_command.instruction_type = lib::SET_GET;//TODO sprawdzic czy to moze byc robione tylko raz
	...
	if (vsp_report == lib::sensor::VSP_REPLY_OK) {
		...
		if (fabs(u[0]) < u_stop[0] / 5 && fabs(u[1]) < u_stop[1] / 5 && fabs(u[2]) < u_stop[2] / 5 && v[0] <= v_stop[0]
				&& v[1] <= v_stop[1] && v[2] <= v_stop[2] && reached[0] == true && reached[1] == true && reached[2]
				== true && tracking == true) {//TODO przemy, popraw
			//if (z_stop) {
		...
		for (int i = 0; i < MAX_AXES_NUM; i++) {
			...
			if (tracking == true && (v[i] == 0 || (v[i] > 0 && v[i] < v_max_act[i] && change[i] == false && reached[i]
			...
			} else if (v[i] > 0 && (change[i] == true || reached[i] == true || tracking == false || (v[i]
					- v_max_act[i]) > 0.0001)) {//hamowanie
				if (v[i] > v_max_act[i] && (v[i] - v_max_act[i]) / t < a_max[i] && change[i] == false && reached[i]
						== false && tracking == true) {
					v[i] = v_max_act[i];
	...
	//memcpy(the_robot->ecp_command.arm.pf_def.arm_frame, next_position,
...
bool ecp_vis_ib_eih_object_tracker_irp6ot::read_parametres()
{//metoda wczytujaca predkosci, przyspieszenia etc z pliku konfiguracyjnego
	//przykladowe wartosci podane sa w first stepie
	v_max[0] = ecp_t.config.value <double> ("v_max_x");//TODO dorobiapanie wyjtku gdy w pliku konfiguracyjnym nie ma odpowiednich zmiennych
