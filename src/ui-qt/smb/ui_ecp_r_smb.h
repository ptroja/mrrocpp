...
public:
	...
	void
	move_motors(const double final_position[lib::smb::NUM_OF_SERVOS]);
	void
	move_joints(const double final_position[lib::smb::NUM_OF_SERVOS]);
	void
	move_external(const double final_position[6], const double _estimated_time);
	void clear_fault();
	void stop_motors();
