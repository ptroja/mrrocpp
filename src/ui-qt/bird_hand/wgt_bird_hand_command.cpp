...
wgt_bird_hand_command::wgt_bird_hand_command(QString _widget_label,
		mrrocpp::ui::common::Interface& _interface,
		mrrocpp::ui::common::UiRobot *_robot, QWidget *parent) :
			...
			ui(new Ui::wgt_bird_hand_commandClass())
{
	...
	connect(this, SIGNAL(synchro_depended_init_signal()), this,
			SLOT(synchro_depended_init_slot()), Qt::QueuedConnection);
	connect(this, SIGNAL(init_and_copy_signal()), this,
			SLOT(init_and_copy_slot()), Qt::QueuedConnection);
	...
	for (int i = 0; i < robot->number_of_servos; i++)
	{
		...
		QList<QAbstractButton*> buttons_in_group =
				checkboxButtonGroup_Vector[i]->buttons();
...
void wgt_bird_hand_command::init()
{
	try
	{
		...
		mrrocpp::lib::bird_hand::status
				&bhsrs =
						robot->ui_ecp_robot->bird_hand_status_reply_data_request_port->data;
		...
		mrrocpp::lib::bird_hand::command &bhcs =
				robot->ui_ecp_robot->bird_hand_command_data_port->data;
	...
	if (robot->state.edp.pid != -1)
	{
		...
		if (robot->state.edp.is_synchronised)
		{
			...
			for (int i = 0; i < robot->number_of_servos; i++)
			{
				...
				joint_command[i]->desired_position
						= desired_pos_spin_box[i]->value();
			}
		}
		else
		{
...
{
	QList<QAbstractButton*> buttons_in_group =
			buttonGroup_Vector[fingerId]->buttons();
	...
	for (int i = 0; i < buttons_in_group.size(); i++)
	{
		if (buttons_in_group[i]->isChecked())
		{
			...
			{
			case 0:
				joint_command[fingerId]->profile_type
						= lib::bird_hand::MACROSTEP_ABSOLUTE_POSITION;
				break;
			case 1:
				joint_command[fingerId]->profile_type
						= lib::bird_hand::MACROSTEP_POSITION_INCREMENT;
				break;
			case 2:
				joint_command[fingerId]->profile_type
						= lib::bird_hand::SIGLE_STEP_POSTION_INCREMENT;
				break;
			default:
				break;
			}
...
{
	joint_command[fingerId]->desired_position
			= desired_pos_spin_box[fingerId]->value();
	joint_command[fingerId]->desired_torque
			= doubleSpinBox_destor_Vector[fingerId]->value();
	joint_command[fingerId]->reciprocal_of_damping
			= doubleSpinBox_rdamp_Vector[fingerId]->value();
...
{
	QList<QAbstractButton*> chboxes =
			checkboxButtonGroup_Vector[fingerId]->buttons();

	doubleSpinBox_curpos_Vector[fingerId]->setValue(
			joint_status[fingerId]->meassured_position);
	doubleSpinBox_curtor_Vector[fingerId]->setValue(
			joint_status[fingerId]->meassured_torque);
	doubleSpinBox_mcur_Vector[fingerId]->setValue(
			joint_status[fingerId]->measured_current);
...
int wgt_bird_hand_command::copy_finger_command(int fingerId)
{
	...
	if (joint_command[fingerId]->profile_type
			== lib::bird_hand::MACROSTEP_ABSOLUTE_POSITION)
		desired_pos_spin_box[fingerId]->setValue(
				doubleSpinBox_curpos_Vector[fingerId]->value());
