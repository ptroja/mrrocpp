...
void wgt_sbench_voltage_command::read_and_set()
{
	...
	for (int i = 0; i < SBENCH_MAX_ROW; i++) {
		for (int j = 0; j < SBENCH_MAX_COL; j++) {
			docks[i][j]->setChecked(robot->ui_ecp_robot->the_robot->data_request_port.data.voltage_buf.get_value(i, j));
		}
	}
...
void wgt_sbench_voltage_command::reshresh_widgets() {
	...
	for (int i = 0; i < SBENCH_MAX_ROW; i++) {
		for (int j = 0; j < SBENCH_MAX_COL; j++) {
			if (robot->ui_ecp_robot->the_robot->data_request_port.data.voltage_buf.get_value(i, j)) {
