...
wgt_sbench_voltage_command::wgt_sbench_voltage_command(QString _widget_label, mrrocpp::ui::common::Interface& _interface, mrrocpp::ui::common::UiRobot *_robot, QWidget *parent) :
		wgt_sbench_command(_widget_label, _interface, _robot, parent)
{
}

wgt_sbench_voltage_command::~wgt_sbench_voltage_command()
{
...
void wgt_sbench_voltage_command::read_and_set()
{
	...
	for (int i = 0; i < SBENCH_MAX_ROW; i++) {
		for (int j = 0; j < SBENCH_MAX_COL; j++) {
			docks[i][j]->setChecked(robot->ui_ecp_robot->the_robot->data_request_port.data.voltage_buf.get_value(i+1, j+1));
		}
	}
...
void wgt_sbench_voltage_command::execute()
{
	...
	for (int i = 0; i < SBENCH_MAX_ROW; i++) {
		for (int j = 0; j < SBENCH_MAX_COL; j++) {
			robot->ui_ecp_robot->the_robot->power_supply_data_port.data.set_value(i+1, j+1, docks[i][j]->isChecked());
		}
	}
...
void wgt_sbench_voltage_command::reshresh_widgets() {
	...
	for (int i = 0; i < SBENCH_MAX_ROW; i++) {
		for (int j = 0; j < SBENCH_MAX_COL; j++) {
			if (robot->ui_ecp_robot->the_robot->data_request_port.data.voltage_buf.get_value(i+1, j+1)) {
