...
//	menuRobot->addSeparator();
	...
	menuTask->addAction(actionConfiguration);
...
void MenuBar::makeConnections()
{
	...
	connect(actionConfiguration, 		SIGNAL(triggered()), this, SLOT(on_actionConfiguration_triggered()), 		Qt::QueuedConnection);
...
void MenuBar::on_actionConfiguration_triggered()
{
	/*
	 QFileDialog dialog;
	 if (dialog.exec()) {
