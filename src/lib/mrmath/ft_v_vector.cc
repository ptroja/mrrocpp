...
Ft_vector::Ft_vector(const Ft_vector & wzor) :
	Ft_v_vector(wzor)
{
}
...
Xyz_Angle_Axis_vector::Xyz_Angle_Axis_vector(const Xyz_Angle_Axis_vector & wzor) :
	Ft_v_vector(wzor)
{
}
...
void Xyz_Angle_Axis_vector::position_distance(const Homog_matrix& local_current_end_effector_frame, const Homog_matrix& local_desired_end_effector_frame)
{
	...
	w[0] = (0.5) * ((n_t[1] * n_d[2] - n_t[2] * n_d[1]) + (o_t[1] * o_d[2] - o_t[2] * o_d[1]) + (a_t[1] * a_d[2]
			- a_t[2] * a_d[1]));
	w[1] = (0.5) * ((n_t[2] * n_d[0] - n_t[0] * n_d[2]) + (o_t[2] * o_d[0] - o_t[0] * o_d[2]) + (a_t[2] * a_d[0]
			- a_t[0] * a_d[2]));
	w[2] = (0.5) * ((n_t[0] * n_d[1] - n_t[1] * n_d[0]) + (o_t[0] * o_d[1] - o_t[1] * o_d[0]) + (a_t[0] * a_d[1]
			- a_t[1] * a_d[0]));
...
Xyz_Euler_Zyz_vector::Xyz_Euler_Zyz_vector(const Xyz_Euler_Zyz_vector & wzor) :
	Ft_v_vector(wzor)
{
}
...
Xyz_Rpy_vector::Xyz_Rpy_vector(const Xyz_Rpy_vector & wzor) :
	Ft_v_vector(wzor)
{
}
