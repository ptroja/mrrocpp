...
namespace NurbsLibImp {
	int solve(const Matrix& AX, const Matrix& BX, Matrix& XX){
		...
		if (AX.cols()!=BX.rows())
			cout<<"solve(AX,BX,XX): wrong matrix sizes cols(AX)="<<AX.cols()<<" rows(BX)="<<BX.rows()<<"\n"<<flush;
		if(AX.cols()==AX.rows()){
...
//tu pewnie ujawnia� si� b��d w metodzie array(). Wczesniej zwracala kopie obiektu a nie referencje.
