...
/* -----------------------------------------------------------------------
  Konstruktor.
 ------------------------------------------------------------------------- */
kinematic_model_irp6ot_with_wrist::kinematic_model_irp6ot_with_wrist (void)
{
...
/* -----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
void kinematic_model_irp6ot_with_wrist::set_kinematic_parameters(void)
{
/* -----------------------------------------------------------------------
Numery osi:
  0 - tor jezdny
...
Polozenia synchronizacji - odczyty z enkoderow silnikow.
 ------------------------------------------------------------------------- */
  ...
  synchro_motor_position[5]= 355.0;		// kisc V [rad] 
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_wrist::check_motor_position(double motor_position[])
{

if (motor_position[0] < lower_limit_axis[0])   // Kat f1 mniejszy od minimalnego
  throw transformer_error::NonFatal_error_2 (BEYOND_LOWER_LIMIT_AXIS_0);
else if (motor_position[0] > upper_limit_axis[0])   // Kat f1 wiekszy od maksymalnego
  throw transformer_error::NonFatal_error_2 (BEYOND_UPPER_LIMIT_AXIS_0);

if (motor_position[1] < lower_limit_axis[1])   // Kat f2 mniejszy od minimalnego
  throw transformer_error::NonFatal_error_2 (BEYOND_LOWER_LIMIT_AXIS_1);
else if (motor_position[1] > upper_limit_axis[1])   // Kat f2 wiekszy od maksymalnego
  throw transformer_error::NonFatal_error_2 (BEYOND_UPPER_LIMIT_AXIS_1);
...
if (motor_position[2] < lower_limit_axis[2])   // Kat f3 mniejszy od minimalnego
  ...
  throw  transformer_error::NonFatal_error_2 (BEYOND_LOWER_LIMIT_AXIS_7);
else if (motor_position[7] > upper_limit_axis[7])   // Kat f8 wiekszy od maksymalnego
  throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_LIMIT_AXIS_7);
    
}; // end: kinematic_model_irp6ot_with_wrist::check_motor_position()


/* ------------------------------------------------------------------------
  Sprawdzenie ograniczen na wspolrzedne wewnetrzne.
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_wrist::check_joints(double q[])
{

	if (isnan(q[0])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_D0);

  if (q[0] < lower_limit_joint[0])  // kat q1 mniejszy od minimalnego
    throw  transformer_error::NonFatal_error_2 (BEYOND_LOWER_D0_LIMIT);
  else if (q[0] > upper_limit_joint[0])   // kat q1 wiekszy od maksymalnego
    throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_D0_LIMIT);

	if (isnan(q[1])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_THETA1);
  if (q[1] < lower_limit_joint[1])  // dlugosc silownika mniejsza od minimalnej
   throw  transformer_error::NonFatal_error_2 (BEYOND_LOWER_THETA1_LIMIT);

  if (q[1] > upper_limit_joint[1])  // dlugosc silownika wieksza od maksymalnej
   throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_THETA1_LIMIT);

	if (isnan(q[2])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_THETA2);
  if (q[2] < lower_limit_joint[2]) // dlugosc silownika mniejsza od minimalnej
   throw  transformer_error::NonFatal_error_2 (BEYOND_LOWER_THETA2_LIMIT);

  if(q[2] > upper_limit_joint[2])  // dlugosc silownika wieksza od maksymalnej
   throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_THETA2_LIMIT);

	if (isnan(q[3])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_THETA3);
  if (q[3] < lower_limit_joint[3] ) // kat q3 mniejszy od minimalnego
   throw  transformer_error::NonFatal_error_2 (BEYOND_LOWER_THETA3_LIMIT);
   
  if (q[3] > upper_limit_joint[3])
    throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_THETA3_LIMIT);

	if (isnan(q[4])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_THETA4);
 ...
 if(q[4] > upper_limit_joint[4])  // kat q4 wiekszy od maksymalnego
   throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_THETA4_LIMIT);
	...
	if (isnan(q[5])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_THETA5);
 if(q[5] < lower_limit_joint[5])  // kat q5 mniejszy od minimalnego
   throw  transformer_error::NonFatal_error_2 (BEYOND_LOWER_THETA5_LIMIT);
 ...
 if(q[5] > upper_limit_joint[5])  // kat q5 wiekszy od maksymalnego
   throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_THETA5_LIMIT);
	...
	if (isnan(q[6])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_THETA6);
 ...
 if(q[6] > upper_limit_joint[6])  // 6 st. swobody   
   throw  transformer_error::NonFatal_error_2 (BEYOND_UPPER_THETA6_LIMIT);   
	  ...
	  //***szczeki chwytaka***
  	if (isnan(q[7])) throw  transformer_error::NonFatal_error_2 (NOT_A_NUMBER_JOINT_VALUE_THETA7);
...
}; // end: kinematic_model_irp6ot_with_wrist::check_joints()
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_wrist::mp2i_transform(double* local_current_motor_pos, double* local_current_joints) {
  ...
  local_current_joints[5] = (local_current_motor_pos[5] - synchro_motor_position[5] - (local_current_motor_pos[4] - synchro_motor_position[4]))/gear[5] + theta[5];
  ...
  local_current_joints[7] = dir_a_7*(local_current_motor_pos[7]*local_current_motor_pos[7]) - dir_b_7*local_current_motor_pos[7] + dir_c_7;
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_wrist::i2mp_transform(double* local_desired_motor_pos_new, double* local_desired_joints)
{
  // Niejednoznacznosc polozenia dla 4-tej osi (obrot kisci < 180ø).
  double joint_4_revolution = M_PI;
  // Niejednoznacznosc polozenia dla 5-tej osi (obrot kisci > 360ø).
  ...
  local_desired_motor_pos_new[3] = gear[3] * sqrt(sl123 + mi3 * cos(local_desired_joints[3]) +
         ni3 * sin(-local_desired_joints[3])) + synchro_joint_position[3];
  ...
  local_desired_motor_pos_new[5] = gear[5] * local_desired_joints[5] + synchro_joint_position[5] + local_desired_motor_pos_new[4];
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_wrist::direct_kinematics_transform(double* local_current_joints, frame_tab* local_current_end_effector_frame_m)
{
  ...
  (*local_current_end_effector_frame_m)[1][3] = s1*(a2*c2+a3*c3+d5*c4) + d0;		//PY
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_wrist::inverse_kinematics_transform(double* local_desired_joints, double* local_current_joints, frame_tab* local_desired_end_effector_frame_m)
{
  ...
  Py= (*local_desired_end_effector_frame_m)[1][3] - local_current_joints[0];
  ...
  if (fabs(s5) < EPS)
  {
	...
	if (fabs(t_ok-local_current_joints[6]) > fabs(t6-M_PI+local_desired_joints[4]-(local_current_joints[6])))
		t_ok=t6-M_PI+local_desired_joints[4];
	if (fabs(t_ok-local_current_joints[6]) > fabs(t6+M_PI+local_desired_joints[4]-(local_current_joints[6])))
		t_ok=t6+M_PI+local_desired_joints[4];

	if (fabs(t_ok-local_current_joints[6]) > fabs(t6-2*M_PI+local_desired_joints[4]-(local_current_joints[6])))
