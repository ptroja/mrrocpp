...
/* -----------------------------------------------------------------------
  Konstruktor.
 ------------------------------------------------------------------------- */
kinematic_model_irp6ot_with_track::kinematic_model_irp6ot_with_track (void)
{
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_track::direct_kinematics_transform(double* local_current_joints, frame_tab* local_current_end_effector_frame_m) {
  ...
  (*local_current_end_effector_frame_m)[0][3] = d6*c4*c1 + a3*c3*c1 + a2*c1*c2;
  ...
  (*local_current_end_effector_frame_m)[1][3] = d6*c4*s1 + a3*c3*s1 + a2*s1*c2 + d0;
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------ */
void kinematic_model_irp6ot_with_track::inverse_kinematics_transform(double* local_desired_joints, double* local_current_joints, frame_tab* local_desired_end_effector_frame_m)
{
  // Stale 
...
// 1a) Kisc zwrocona do pionowo gory, theta4=-pi/2
  if ( (*local_desired_end_effector_frame_m)[2][2] > 0 && ABS((*local_desired_end_effector_frame_m)[2][0]) <= EPS && ABS((*local_desired_end_effector_frame_m)[2][1]) <= EPS ) {
    std::cout << " Osobliwosc! (theta4=-pi/2)" << std::endl;
    p=true;
    local_desired_joints[1] = M_PI-atan2( (*local_desired_end_effector_frame_m)[1][3]-local_desired_joints[0], -(*local_desired_end_effector_frame_m)[0][3] );
    local_desired_joints[5] = atan2( -(*local_desired_end_effector_frame_m)[1][0], -(*local_desired_end_effector_frame_m)[0][0] )-local_desired_joints[1];
...
// 1b) Kisc zwrocona do pionowo do dolu, theta4= pi/2
  if ( (*local_desired_end_effector_frame_m)[2][2] < 0 && ABS((*local_desired_end_effector_frame_m)[2][0])<=EPS && ABS((*local_desired_end_effector_frame_m)[2][1])<=EPS ){
    ...
    p=true;
    local_desired_joints[1]=M_PI-atan2( (*local_desired_end_effector_frame_m)[1][3]-local_desired_joints[0], -(*local_desired_end_effector_frame_m)[0][3] );
    local_desired_joints[5]=local_desired_joints[1]-atan2(  (*local_desired_end_effector_frame_m)[1][0],  (*local_desired_end_effector_frame_m)[0][0] );
    local_desired_joints[4]=M_PI_2;
  }

// 2a) Ramie robota w osi toru -- theta[1]=pi/2
  if ( !p && ABS((*local_desired_end_effector_frame_m)[0][3])<=EPS && (*local_desired_end_effector_frame_m)[1][2]>0){
    std::cout << " Osobliwosc! (theta1=pi/2)" << std::endl;
    local_desired_joints[5] = atan2 ( (*local_desired_end_effector_frame_m)[0][0], (*local_desired_end_effector_frame_m)[0][1] );
    local_desired_joints[4] = atan2 ( -(*local_desired_end_effector_frame_m)[2][2], (*local_desired_end_effector_frame_m)[1][2] );
...
if (!p) {
  local_desired_joints[1] = atan2(  (*local_desired_end_effector_frame_m)[1][2], (*local_desired_end_effector_frame_m)[0][2] );
  local_desired_joints[4] = atan2( -(*local_desired_end_effector_frame_m)[2][2], (*local_desired_end_effector_frame_m)[0][2]/cos(local_desired_joints[1]) );
  local_desired_joints[5] = atan2( -(*local_desired_end_effector_frame_m)[2][1], (*local_desired_end_effector_frame_m)[2][0] );
  local_desired_joints[0] = -(*local_desired_end_effector_frame_m)[0][3]*sin(local_desired_joints[1])/cos(local_desired_joints[1])+
	(*local_desired_end_effector_frame_m)[1][3];
}
  ...
  double px1 = (*local_desired_end_effector_frame_m)[0][3] - d6*c4*cos(local_desired_joints[1]);
  double py1 = (*local_desired_end_effector_frame_m)[1][3] - d6*c4*sin(local_desired_joints[1])-local_desired_joints[0];
  double pz1 = (*local_desired_end_effector_frame_m)[2][3] + d6*s4-d1;
  double d = sqrt(px1*px1+py1*py1+pz1*pz1);
  double alfa = asin(pz1/d);

  local_desired_joints[2] = -alfa - acos((a2_2+d*d-a3_2)/(2*a2*d));
  local_desired_joints[3] = asin((-d6*s4-a2*sin(local_desired_joints[2])+d1 - (*local_desired_end_effector_frame_m)[2][3] )/a3);
