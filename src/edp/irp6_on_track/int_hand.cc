...
extern in_out_buffer in_out_obj; // bufor wejsc wyjsc
...
int_handler (void *arg, int int_id) 
{
	...
	else if (md.interrupt_mode == INT_SERVOING)
	{
		...
		in_out_obj.set_input(&binary_input, analog_input);
		
		// ustawienie wyjscia o ile bylo takie zlecenie
		if (in_out_obj.set_output_flag)
		{
			in_out_obj.set_output_flag=false;
			in_out_obj.get_output(&binary_output);
