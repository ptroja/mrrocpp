...
servo_buffer::servo_buffer(effector &_master) :
	...
	for (int j = 0; j < master.number_of_servos; j++) {
		synchro_axis_order[j] = ((j + IRP6OT_SYN_INIT_AXE)
		...
		default:
			...
			synchro_step_coarse[j]
					= IRP6_ON_TRACK_AXIS_0_TO_5_SYNCHRO_STEP_COARSE;
...
void servo_buffer::load_hardware_interface(void) {
	...
	int max_current[IRP6OT_M_NUM_OF_SERVOS] = {
			...
			IRP6_ON_TRACK_AXIS_7_MAX_CURRENT };
	...
	regulator_ptr[1] = new NL_regulator_2_irp6ot(0, 0, 0.429, 6.834, 6.606,
			0.35, master);
	// Serwomechanizm 3
	regulator_ptr[2] = new NL_regulator_3_irp6ot(0, 0, 0.64, 9.96 / 4,
			9.54 / 4, 0.35, master);
	// Serwomechanizm 4
	// regulator_ptr[3] = new NL_regulator_4 (0, 0, 0.62, 9.85/4, 9.39/4, 0.35);
	regulator_ptr[3] = new NL_regulator_4_irp6ot(0, 0, 0.333, 5.693, 5.427,
			0.35, master);
	// Serwomechanizm 5
	regulator_ptr[4] = new NL_regulator_5_irp6ot(0, 0, 0.56, 7.98 / 2,
			7.55 / 2, 0.35, master);
	// Serwomechanizm 6
	// regulator_ptr[5] = new NL_regulator_6 (0, 0, 0.3079*2, 0.6, 0.6, 0.35);
	regulator_ptr[5] = new NL_regulator_6_irp6ot(0, 0, 0.39, 8.62 / 2., 7.89
			/ 2., 0.35, master);

	regulator_ptr[6] = new NL_regulator_7_irp6ot(0, 0, 0.39, 8.62 / 2., 7.89
			/ 2., 0.35, master);
