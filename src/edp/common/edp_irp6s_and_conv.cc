...
/*--------------------------------------------------------------------------*/
edp_irp6s_and_conv_effector::edp_irp6s_and_conv_effector (ROBOT_ENUM l_robot_name) :
	 edp_effector (l_robot_name), kinematics_manager(), step_counter(0)
{
...
void edp_irp6s_and_conv_effector::move_arm (c_buffer *instruction){};            // przemieszczenie ramienia

void edp_irp6s_and_conv_effector::get_arm_position (bool read_hardware, c_buffer *instruction){}; // odczytanie pozycji ramienia

void edp_irp6s_and_conv_effector::servo_joints_and_frame_actualization_and_upload(void) {}; // by Y

bool edp_irp6s_and_conv_effector::is_power_on() const { return controller_state_edp_buf.is_power_on;};
...
void edp_irp6s_and_conv_effector::arm_joints_2_joints (void) {
	...
	switch (reply.reply_type) {
		...
		case ARM_RMODEL:
		case ARM_RMODEL_INPUTS:
			for (int i=0; i<number_of_servos; i++) reply.arm.coordinate_def.arm_coordinates[i] = current_joints[i];
			break;
		default: // blad:
			throw NonFatal_error_2(STRANGE_GET_ARM_REQUEST);
	}
...
void edp_irp6s_and_conv_effector::set_rmodel (c_buffer *instruction) {};                    // zmiana narzedzia
void edp_irp6s_and_conv_effector::get_rmodel (c_buffer *instruction) {};                    // odczytanie narzedzia
...
// ustalenie formatu odpowiedzi
 ...
 if (is_get_arm(instruction)) {
   switch (reply.reply_type) {
    case ACKNOWLEDGE: reply.reply_type = ARM;                 break;
    case INPUTS:        reply.reply_type = ARM_INPUTS;        break;
    case RMODEL:           reply.reply_type = ARM_RMODEL;           break;
    case RMODEL_INPUTS: reply.reply_type = ARM_RMODEL_INPUTS; break;
    default:
    break;
   }; // end: switch
   }
 real_reply_type = reply.reply_type;
 if (is_set_arm(instruction)) {// by Y ORIGINAL
...
void edp_irp6s_and_conv_effector::compute_joints (c_buffer *instruction) {
 ...
 /* Zlecenie transformerowi przeliczenie wspolrzednych */
	...
	switch (motion_type) {
	case ABSOLUTE:   // ruch bezwzgledny
		for (i=0; i<number_of_servos; i++) desired_joints_tmp[i] = p[i];
		break;
	case RELATIVE:   // ruch wzgledny
		for (i=0; i<number_of_servos; i++) desired_joints_tmp[i] = desired_joints[i] + p[i];
		break;
     default:
        	throw NonFatal_error_2(INVALID_MOTION_TYPE);
	}
	...
	for (i=0; i<number_of_servos; i++) previous_joints[i] = desired_joints[i];
...
//            printf("SET_GET\n");
       ...
       break;
    case WAIT:
	         if ( receive_instruction() == QUERY ) { // instrukcja wlasciwa =>
...
try { // w tym bloku beda wylapywane wyjatki (bledy)
 switch (next_state) {
   ...
   case SYNCHRO_TERMINATED:
     ...
     if ( receive_instruction() == QUERY ) { // instrukcja wlasciwa => zle jej wykonanie
