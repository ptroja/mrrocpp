...
void *reader_thread(void* arg)
{
	...
	for (;;) {
		...
		while (!wyjscie) {
			...
			MsgReply(rcvid, EOK, 0, 0);    rcvid = MsgReceive(my_attach->chid, &ui_msg, sizeof(ui_msg), NULL);
		...
		if (!outfile)  // jesli plik nie instnieje
		{
			...
			for (i = 0; i < msr_counter; i++)
			{
				...
				if (rb_obj.reader_cnf.msec) outfile << r_measptr[k].msec << " ";
				if (rb_obj.reader_cnf.servo_tryb) outfile << r_measptr[k].servo_tryb << " ";
					
				for (int j=0; j<MAX_SERVOS_NR; j++)
				{	
					if (rb_obj.reader_cnf.desired_inc[j]) outfile << r_measptr[k].desired_inc[j] << " ";
					if (rb_obj.reader_cnf.current_inc[j]) outfile << r_measptr[k].current_inc[j] << " ";
					if (rb_obj.reader_cnf.pwm[j]) outfile << r_measptr[k].pwm[j] << " ";
					if (rb_obj.reader_cnf.uchyb[j]) outfile << r_measptr[k].uchyb[j] << " ";
					if (rb_obj.reader_cnf.abs_pos[j]) outfile << r_measptr[k].abs_pos[j] << " ";
				}
				...
				for (int j=0; j<MAX_SERVOS_NR; j++)
				{	
					if (rb_obj.reader_cnf.current_joints[j]) outfile << r_measptr[k].current_joints[j] << " ";
				}
				...
				for (int j=0; j<6; j++)
				{	
					if (rb_obj.reader_cnf.force[j]) outfile << r_measptr[k].force[j] << " ";
					if (rb_obj.reader_cnf.desired_force[j]) outfile << r_measptr[k].desired_force[j] << " ";
					if (rb_obj.reader_cnf.filtered_force[j]) outfile << r_measptr[k].filtered_force[j] << " ";
				}
				...
				for (int j=0; j<6; j++)
				{	
					if (rb_obj.reader_cnf.current_kartez_position[j]) outfile << r_measptr[k].current_kartez_position[j] << " ";
				}
				...
				for (int j=0; j<6; j++)
				{	
					if (rb_obj.reader_cnf.real_kartez_position[j]) outfile << r_measptr[k].real_kartez_position[j] << " ";
				}
				...
				for (int j=0; j<6; j++)
				{	
					if (rb_obj.reader_cnf.real_kartez_vel[j]) outfile << r_measptr[k].real_kartez_vel[j] << " ";
				}
				...
				for (int j=0; j<6; j++)
				{	
					if (rb_obj.reader_cnf.real_kartez_acc[j]) outfile << r_measptr[k].real_kartez_acc[j] << " ";
				}
