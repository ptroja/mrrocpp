...
/*--------------------------------------------------------------------------*/
edp_irp6s_postument_track_effector::edp_irp6s_postument_track_effector (ROBOT_ENUM l_robot_name) : 
	edp_irp6s_effector (l_robot_name)
{
	...
	switch (reply.reply_type) {
		...
		case ARM_RMODEL_INPUTS:
		{
			if(*read_hardware){
				...
				copy_frame(reply.arm.pose_force_torque_at_frame_def.beggining_arm_frame_m, current_end_effector_frame_m);
				copy_frame(reply.arm.pose_force_torque_at_frame_def.predicted_arm_frame_m, current_end_effector_frame_m);
				copy_frame(reply.arm.pose_force_torque_at_frame_def.present_arm_frame_m, current_end_effector_frame_m);
			}
...
/*
	...
	*/
	...
	static frame_tab local_force_end_effector_frame_m;
...
//		printf("a rx: %f, ry: %f, rz: %f\n", move_vector[0], move_vector[1], move_vector[2]);
		...
		*/
		...
		next_frame.get_frame_tab(desired_end_effector_frame_m);
		...
		{ // przygotowanie predicted frame dla ECP
			next_frame.get_frame_tab(reply.arm.pose_force_torque_at_frame_def.present_arm_frame_m);
			...
			predicted_frame.get_frame_tab(reply.arm.pose_force_torque_at_frame_def.predicted_arm_frame_m);
...
/*--------------------------------------------------------------------------*/
void edp_irp6s_postument_track_effector::move_arm (c_buffer *instruction) { // przemieszczenie ramienia
...
/*--------------------------------------------------------------------------*/
void edp_irp6s_postument_track_effector::get_arm_position (bool read_hardware, c_buffer *instruction) { // odczytanie pozycji ramienia
...
{// by Y
	...
	if (translation_mode == WITHOUT_TRANSLATION) return_frame.remove_translation();
	return return_frame;
}

void edp_irp6s_postument_track_effector::force_msr_upload(double *new_value){// by Y wgranie globalnego zestawu danych
...
void edp_irp6s_postument_track_effector::force_msr_download(double *new_value, double *old_value)
{
	...
	for (int i=0;i<=5;i++)
	{
		if (new_value) new_value[i]=global_kartez_force_msr[i];
		if (old_value) old_value[i]=prevoius_global_kartez_force_msr[i];
	}
	pthread_mutex_unlock( &force_mutex );
}
