...
void edp_irp6s_postument_track_effector::pose_force_torque_at_frame_move(c_buffer &instruction)
{
	...
	if (motion_type == ABSOLUTE) {
		...
		switch (instruction.set_arm_type) {
			case XYZ_ANGLE_AXIS:
				...
				break;
			case JOINT:
				get_current_kinematic_model()->i2e_transform(instruction.arm.pf_def.arm_coordinates, &goal_frame_tab);
				goal_frame.set_frame_tab(goal_frame_tab);
				break;
			case MOTOR:
				get_current_kinematic_model()->mp2i_transform(instruction.arm.pf_def.arm_coordinates, tmp_joints);
	...
	switch (instruction.set_arm_type) {
		...
		case MOTOR:
			...
			for (int i = 0; i < 6; i++) {
				pos_xyz_rot_xyz[i] =goal_xyz_angle_axis_increment_in_end_effector[i] * (double) (1/ ( ((double)STEP)
