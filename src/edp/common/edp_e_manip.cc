...
bool manip_effector::compute_servo_joints_and_frame(void) {
	...
	if (!(motor_driven_effector::compute_servo_joints_and_frame())) {
		...
		try {
			...
			get_current_kinematic_model()->i2e_transform(servo_current_joints,
					local_matrix);
			...
			lib::MotorArray servo_desired_motor_pos(
					sb->command.parameters.move.abs_position, number_of_servos);

			lib::JointArray servo_desired_joints(number_of_servos);

			get_current_kinematic_model()->mp2i_transform(
					servo_desired_motor_pos, servo_desired_joints);
			get_current_kinematic_model()->i2e_transform(servo_desired_joints,
					local_matrix);
			// Pobranie wsp. zewn��trznych w uk��adzie
			...
			// scope-locked reader data update
			{
				...
				servo_real_kartez_pos.to_table(
						rb_obj->step_data.real_cartesian_position);
				servo_desired_kartez_pos.to_table(
						rb_obj->step_data.desired_cartesian_position);
			}

			// Obliczenie polozenia robota we wsp. zewnetrznych bez narzedzia.
			((mrrocpp::kinematics::common::kinematic_model_with_tool*) get_current_kinematic_model())->i2e_wo_tool_transform(
					servo_current_joints, servo_current_frame_wo_tool);
			...
			if ((++catch_nr) == 1)
				printf(
						"servo thread compute_servo_joints_and_frame throw catch exception\n");
			ret_val = false;
		}//: catch
	}
...
/*--------------------------------------------------------------------------*/
void manip_effector::get_arm_position_with_force_and_sb(bool read_hardware,
	...
	if (read_hardware) {
		...
		if (is_synchronised()) {
			...
			get_current_kinematic_model()->mp2i_transform(
					desired_motor_pos_new, desired_joints_tmp);
	...
	get_current_kinematic_model()->mp2i_transform(current_motor_pos,
			current_joints);
	get_current_kinematic_model()->i2e_transform(current_joints,
			current_end_effector_frame);
...
/*--------------------------------------------------------------------------*/
void manip_effector::get_arm_position_get_arm_type_switch(
		lib::c_buffer &instruction) { // odczytanie pozycji ramienia
...
/*--------------------------------------------------------------------------*/
void manip_effector::compute_base_pos_xyz_rot_xyz_vector(
	...
	lib::JointArray joint_arm_coordinates(
			instruction.arm.pf_def.arm_coordinates, number_of_servos);
	lib::MotorArray motor_arm_coordinates(
			instruction.arm.pf_def.arm_coordinates, number_of_servos);
	...
	case lib::ABSOLUTE:
		...
		case lib::FRAME:
			...
			break;
		case lib::JOINT:
			get_current_kinematic_model()->i2e_transform(joint_arm_coordinates,
					goal_frame);

			break;
		case lib::MOTOR:
			get_current_kinematic_model()->mp2i_transform(
					motor_arm_coordinates, tmp_joints);
	...
	case lib::RELATIVE:
		...
		case lib::MOTOR:
			for (int i = 0; i < number_of_servos; i++) {
				tmp_motor_pos[i] = desired_motor_pos_new[i]
						+ motor_arm_coordinates[i];
			}
			get_current_kinematic_model()->mp2i_transform(tmp_motor_pos,
					tmp_joints);
			get_current_kinematic_model()->i2e_transform(tmp_joints, goal_frame);
			break;
		default:
	...
	case lib::JOINT:
	case lib::MOTOR:
		goal_frame_increment_in_end_effector = ((!begining_end_effector_frame)
				* goal_frame);
		goal_frame_increment_in_end_effector.get_xyz_angle_axis(
				goal_xyz_angle_axis_increment_in_end_effector);
		for (int i = 0; i < 6; i++) {
			base_pos_xyz_rot_xyz_vector[i]
					= goal_xyz_angle_axis_increment_in_end_effector[i]
							* (double) (1 / (((double) STEP)
									* ((double) ECP_motion_steps)));
		}
		break;
	case lib::PF_VELOCITY:
		base_pos_xyz_rot_xyz_vector.set_values(
				instruction.arm.pf_def.arm_coordinates);
		break;
	default:
		throw System_error();
	}
...
/*--------------------------------------------------------------------------*/
void manip_effector::iterate_macrostep(const lib::JointArray & begining_joints,
		const lib::Homog_matrix & begining_end_effector_frame,
		const lib::c_buffer &instruction,
	...
	const lib::BEHAVIOUR_SPECIFICATION (&behaviour)[6] =
			instruction.arm.pf_def.behaviour;
	...
	for (int i = 0; i < 6; i++) {
		...
		case lib::UNGUARDED_MOTION:
			...
			force_xyz_torque_xyz[i]
					= instruction.arm.pf_def.force_xyz_torque_xyz[i];
			// inertia is not eleliminated
			break;
		case lib::GUARDED_MOTION:
			inertia[i] = instruction.arm.pf_def.inertia[i];
			reciprocal_damping[i]
					= instruction.arm.pf_def.reciprocal_damping[i];
	...
	for (int step = 1; step <= ECP_motion_steps; ++step) {
		...
		lib::Homog_matrix begining_end_effector_frame_with_current_translation =
				begining_end_effector_frame;
		begining_end_effector_frame_with_current_translation.set_translation_vector(
				desired_end_effector_frame);

		lib::Ft_v_vector current_force_torque(ft_tr_inv_tool_matrix
				* !(lib::Ft_tr(current_frame_wo_offset)) * current_force);
		...
		if (step_counter - last_force_step_counter
		...
		previous_move_rot_vector = v_tr_inv_tool_matrix
				* (!v_tr_current_frame_matrix) * previous_move_rot_vector;

		switch (set_arm_type) {
		case lib::FRAME:
		case lib::JOINT:
		case lib::MOTOR:
			pos_xyz_rot_xyz_vector = lib::V_tr(!(lib::V_tr(
					!begining_end_effector_frame_with_current_translation
							* desired_end_effector_frame)))
		...
		for (int i = 0; i < 6; i++) {
			...
			// PRAWO STEROWANIA
			move_rot_vector[i] = ((reciprocal_damping[i]
					* (force_xyz_torque_xyz[i] - current_force_torque[i])
					+ pos_xyz_rot_xyz_vector[i]) * STEP * STEP
					+ reciprocal_damping[i] * inertia[i]
							* previous_move_rot_vector[i]) / (STEP
					+ reciprocal_damping[i] * inertia[i]);
		}

		previous_move_rot_vector = v_tr_current_frame_matrix * v_tr_tool_matrix
				* move_rot_vector;
		...
		get_current_kinematic_model()->e2i_transform(desired_joints_tmp,
				desired_joints, desired_end_effector_frame);
		// Przeliczenie wspolrzednych wewnetrznych na polozenia walow silnikow

		get_current_kinematic_model()->i2mp_transform(
				desired_motor_pos_new_tmp, desired_joints_tmp);
...
/*--------------------------------------------------------------------------*/
void manip_effector::pose_force_torque_at_frame_move(
		const lib::c_buffer &instruction) {
	// WYLICZENIE POZYCJI POCZATKOWEJ
	lib::JointArray begining_joints(number_of_servos);
	lib::Homog_matrix begining_end_effector_frame;
	get_current_kinematic_model()->mp2i_transform(desired_motor_pos_new,
			begining_joints);
	get_current_kinematic_model()->i2e_transform(begining_joints,
			begining_end_effector_frame);

	lib::Xyz_Angle_Axis_vector base_pos_xyz_rot_xyz_vector; // wartosci ruchu pozycyjnego

	// WYZNACZENIE base_pos_xyz_rot_xyz_vector
	compute_base_pos_xyz_rot_xyz_vector(begining_joints,
			begining_end_effector_frame, instruction,
			base_pos_xyz_rot_xyz_vector);

	// macrostep iteration
	iterate_macrostep(begining_joints, begining_end_effector_frame,
...
void manip_effector::set_robot_model(const lib::c_buffer &instruction) {
	...
	case lib::FORCE_TOOL:
		...
		}
		{
			...
			for (int i = 0; i < 3; i++) {
				vs->next_force_tool_position[i]
						= instruction.robot_model.force_tool.position[i];
			}
			vs->next_force_tool_weight
					= instruction.robot_model.force_tool.weight;
			vs->new_edp_command = true;
		}
...
void manip_effector::get_robot_model(lib::c_buffer &instruction) {
	...
	case lib::FORCE_TOOL:
		...
		for (int i = 0; i < 3; i++) {
			reply.robot_model.force_tool.position[i]
					= vs->current_force_tool_position[i];
		}
	...
	case lib::TOOL_FRAME:
		...
		((mrrocpp::kinematics::common::kinematic_model_with_tool*) get_current_kinematic_model())->tool.get_frame_tab(
...
void manip_effector::compute_frame(const lib::c_buffer &instruction) {
	...
	if ((value_in_step_no <= 0) || (motion_steps <= 0) || (value_in_step_no
			> motion_steps + 1)) {
	...
	get_current_kinematic_model()->e2i_transform(desired_joints_tmp,
			desired_joints, desired_end_effector_frame);
	// Przeliczenie wspolrzednych wewnetrznych na polozenia walow silnikow
	get_current_kinematic_model()->i2mp_transform(desired_motor_pos_new_tmp,
			desired_joints_tmp);
...
lib::Homog_matrix manip_effector::return_current_frame(
		TRANSLATION_ENUM translation_mode) {// by Y
...
void manip_effector::force_msr_upload(const lib::Ft_vector & l_vector) {// by Y wgranie globalnego zestawu danych
...
/*--------------------------------------------------------------------------*/
void manip_effector::single_thread_move_arm(const lib::c_buffer &instruction) { // przemieszczenie ramienia
...
/*--------------------------------------------------------------------------*/
void manip_effector::multi_thread_move_arm(const lib::c_buffer &instruction) { // przemieszczenie ramienia
	// Wypenienie struktury danych transformera na podstawie parametrow polecenia
	// otrzymanego z ECP. Zlecenie transformerowi przeliczenie wspolrzednych
