...
/*-----------------------------------------------------------------------*/
conveyor_servo_buffer::conveyor_servo_buffer (void) : servo_buffer() {
  ...
  // Serwomechanizm 1
  regulator_ptr[0] = new NL_regulator_1_irp6p (0, 0, 0.333, 6.2, 5.933, 0.35); // tasmociag dla irp6 postument
...
{
	return new conveyor_servo_buffer ();
}
