...
edp_speaker_effector::edp_speaker_effector () : edp_effector(ROBOT_SPEAKER)
{
...
void edp_speaker_effector::get_spoken (bool read_hardware, c_buffer *instruction) { // MAC7
return;
};

int edp_speaker_effector::speak (c_buffer *instruction) { // add by MAC7
   ...
   case GET_INSTRUCTION:
     switch ( receive_instruction() ) {
      ...
      default: // blad: nieznana instrukcja
        ...
        if (is_reply_type_ERROR()) printf("ERROR GET_INSTRUCTION 2 aaa\n");
...
{
	return new edp_speaker_effector ();
};
