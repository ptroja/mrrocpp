...
//		  chCharInR = sText.at(stI+2); //jeœli spacja, to jeden znak dalej
		...
		{
		 if((sText.at(stI+2)=='a')||(sText.at(stI+2)=='e')||(sText.at(stI+2)=='o')||(sText.at(stI+2)=='u')||(sText.at(stI+2)=='ê')||(sText.at(stI+2)=='¹')||(sText.at(stI+2)=='i')||(sText.at(stI+2)=='y'))
			chCharInR2 = 1;
		 if((sText.at(stI+2)=='i'))
			chCharInR2 = 2;
		}
		...
		case 'ê':	
			...
			if ((chCharInR == 'c')||(chCharInR == 't')||(chCharInR == 'd'))
				...
				AddPhoneme("n'",&vphoPhonemes);
			else if (!((chCharInR == 'l')||(chCharInR == '³')||(chCharInR == ' ')||(chCharInR == '.')||(chCharInR == ',')||(chCharInR == '?')))
				AddPhoneme("N",&vphoPhonemes);
			break;
		...
		case 'y':	AddPhoneme("I", &vphoPhonemes);		break;
		case 'æ':	AddPhoneme("Ts",&vphoPhonemes);		break;
		case '³':	AddPhoneme("w", &vphoPhonemes);		break;
...
//phrase boundary syllables
	...
	for (stI = vphoPhonemes.size(); stI > 0; stI--)
	{
		...
		if (cLastPhonemeCategory != 1) //poprzendni nie samog³oska
			if ((phoPhoneme.cCategory != 1) && (phoPhoneme.cCategory < 100)) //nie samog³oska, nie znak przest.
				phoPhoneme.cHalfSyllable = 1;	//fonem "po³ówkowy"
	...
	for (stI = 1; stI < vphoPhonemes.size(); stI++)	//zero dla jednosylabowych
	{
		...
		if (!phoPhoneme.sName.compare("|")) phoPhoneme.sName = "#";
		vphoPhonemes.at(stI) = phoPhoneme;
	}
	...
	if (pLabFile.is_open()) {
		...
		while (!pLabFile.eof()) {
			...
			if (phoTemp.sName != sLine.substr(stFrom, stTo)) {
				cout << "Error in line " << iLine << " of corpus.nlp. PhonemePlus name does not match corpus.lab" << endl;
				exit(1);
			}
			...
			} else phoTemp.cSyllableFromWordEnd = 0;
			if (stTo != string::npos) {
				stFrom = stTo + 1;
				stTo = sLine.find('\t', stFrom);
				phoTemp.cPhraseBoundary = atoi(sLine.substr(stFrom, stTo-stFrom).c_str());
			} else phoTemp.cPhraseBoundary = 0;
			if (stTo != string::npos) {
				stFrom = stTo + 1;
				stTo = sLine.find('\t', stFrom);
				phoTemp.cStress = atoi(sLine.substr(stFrom, stTo-stFrom).c_str());
			} else phoTemp.cStress = 0;

			// assign fundamental frequency from the F0 samples vector
			unsigned int iStart = static_cast<unsigned int> ( ceil ((static_cast<float> (phoTemp.iBegSec) + static_cast<float> (phoTemp.iBegMSec)/10000000 - config_fF0SamplingOffset) / config_fF0Sampling) );
			unsigned int iEnd = static_cast<unsigned int> ( floor ((static_cast<float> (phoTemp.iEndSec) + static_cast<float> (phoTemp.iEndMSec)/10000000 - config_fF0SamplingOffset) / config_fF0Sampling) );
			...
			if (phoTemp.bVoiced) {
				...
				iBuffer = 0;
				while (vcF0[iStart + iBuffer] == 0 && iBuffer < iMaxBuffer) iBuffer++;
				phoTemp.iF0start = vcF0[iStart + iBuffer];

				iBuffer = 0;
				while (vcF0[iEnd - iBuffer] == 0 && iBuffer < iMaxBuffer) iBuffer++;
			...
			if (phoTemp.bVoiced && (vcF0[iStart] == 0 || vcF0[iEnd] == 0)) iCountF0Errors++;
			if (phoTemp.bVoiced && (phoTemp.iF0start == 0 || phoTemp.iF0end == 0)) iCountF0ErrorsAfterCorrections++;
...
vector<Unit> fnTextIntoSyllables(const vector<Phoneme>& vphoText)
{
	...
	//	cout << static_cast<int> (vphoText[iText].cHalfSyllable) << " ";
		...
		if (!vuVector.empty() && vphoText[ vuVector.back().viPhonemePlus.back() ].cSyllableFromWordEnd == vphoText[iText].cSyllableFromWordEnd && vuVector.back().iType != -1) {
...
float fnTargetCost(PhonemePlus pCandidate, Phoneme pText)
{
	...
	if (pCandidate.cSyllableFromWordEnd != pText.cSyllableFromWordEnd && pCandidate.cSyllableFromWordEnd != pText.cSyllableFromWordEnd + pText.cHalfSyllable) {
		if (pText.cSyllableFromWordEnd + pCandidate.cSyllableFromWordEnd <= 2) fCost = config_TC_fSyllableMismatchPenalty;	// 0-1, 0-2, 1-0, 2-0 mismatch
		else if (pText.cSyllableFromWordEnd < 3) fCost = config_TC_fEndingSyllableMismatchPenalty;
		else fCost = config_TC_fSyllableMismatchPenalty;
	}
	...
	//		(pCandidate.cPhraseBoundary == 0 && pText.cPhraseBoundary == 2) ||
			...
			(pCandidate.cPhraseBoundary == 0 && pText.cPhraseBoundary == 4) ||
			(pCandidate.cPhraseBoundary == 6 && pText.cPhraseBoundary == 0) ||
			(pCandidate.cPhraseBoundary == 0 && pText.cPhraseBoundary == 6) ||	//AJ end

			(pCandidate.cPhraseBoundary == 3 && pText.cPhraseBoundary == 4) ||
			(pCandidate.cPhraseBoundary == 4 && pText.cPhraseBoundary == 3) ||
			(pCandidate.cPhraseBoundary == 5 && pText.cPhraseBoundary == 6) ||
			(pCandidate.cPhraseBoundary == 6 && pText.cPhraseBoundary == 5)) fCost = config_TC_fPhraseBoundaryNearMismatchPenalty;
		else fCost = config_TC_fPhraseBoundaryFarMismatchPenalty;
	}
...
float fnUnitTargetCost(Unit uCandidate, Unit uText, vector<PhonemePlus>& vphoCorpus, vector<Phoneme>& vphoText)
{
	...
	for (unsigned int iPhonemePlus = 0 ; iPhonemePlus < uCandidate.viPhonemePlus.size() ; iPhonemePlus++) {
		fCost += fnTargetCost(vphoCorpus[ uCandidate.viPhonemePlus[iPhonemePlus] ], vphoText[ uText.viPhonemePlus[iPhonemePlus] ]);
	}
...
float fnConcatenationCost(Unit uPrevious, Unit uCurrent, vector<PhonemePlus>& vphoCorpus)
{
	...
	if (vphoCorpus[ uPrevious.viPhonemePlus.back() ].sName == "#" && vphoCorpus[ uCurrent.viPhonemePlus.front() ].cCategory == 2) {
		// # + plosive UV
		return fCost = 0;
	}
	...
	if (vphoCorpus[ uPrevious.viPhonemePlus.back() ].bVoiced && vphoCorpus[ uCurrent.viPhonemePlus.front() ].bVoiced) {
		unsigned char ucF0difference = abs( vphoCorpus[ uPrevious.viPhonemePlus.back() ].iF0end - vphoCorpus[ uCurrent.viPhonemePlus.front() ].iF0start );
		if (ucF0difference <= 10) fCost += config_CC_fF0Difference_0to10;
		else if (ucF0difference <= 20) fCost += config_CC_fF0Difference_10to20;
		else if (ucF0difference <= 30) fCost += config_CC_fF0Difference_20to30;
		else fCost += config_CC_fF0Difference_30more;
	}
	...
	if (uCurrent.viPhonemePlus.front() - 1 > 0) phoCurrentFirstNeighbour = vphoCorpus[ uCurrent.viPhonemePlus.front() - 1 ];
	else phoCurrentFirstNeighbour.name("#");
	if (uPrevious.viPhonemePlus.back() + 1 < vphoCorpus.size()) phoPreviousLastNeighbour = vphoCorpus[ uPrevious.viPhonemePlus.back() + 1 ];
	else phoCurrentFirstNeighbour.name("#");
	...
	} else fCost += config_CC_fAllTheRest;				// the rest
	...
	if (phoCurrentFirst.cCategory == 1 && 				// samogloska
		phoPreviousLast.bSoft &&
		!phoCurrentFirstNeighbour.bSoft)
			fCost += config_CC_fHighNeighbourInfluencePenalty;
	...
	if ((phoPreviousLastNeighbour.sName == "#" ||
		phoPreviousLastNeighbour.cCategory == 2 ||		// wybuchowe UV
		phoPreviousLastNeighbour.sName == "ts" ||		// zwarto-trace UV
		phoPreviousLastNeighbour.sName == "Ts" ||		// ...
		phoPreviousLastNeighbour.sName == "tS"))
		if(		// ...
		!(phoCurrentFirst.sName == "#" ||
		phoCurrentFirst.cCategory == 2 ||				// wybuchowe UV
...
vector<unsigned int> fnFindBestPath(unsigned int iUnit, unsigned int iWindowSize, vector< vector<Unit> >& vCandidates, vector<PhonemePlus>& vphoCorpus) {
	...
	while (true) {
		...
		for (unsigned int iTemp = 0 ; iTemp < iWindowSize-1 ; iTemp++) {
			...
			fConcatenationCost += fnConcatenationCost(vCandidates[iUnit + iTemp][viCurrentPath[iTemp]], vCandidates[iUnit + iTemp + 1][viCurrentPath[iTemp + 1]], vphoCorpus);
			fCurrentPartTotalCost = (fTargetCost * config_fTargetCostWeight) + (fConcatenationCost * (1 - config_fTargetCostWeight));

			if (fCurrentPartTotalCost >= fTotalCost) {
				// skip this subpath
				for (iTemp += 2 ; iTemp < iWindowSize ; iTemp++) viCurrentPath[iTemp] = static_cast<unsigned int> (vCandidates[iUnit + iTemp].size()) - 1;
				bSkipFlag = true;
				break;
			}
		}
...
vector<Segment> fnGenerateSegments (vector<Unit>& vuVector, vector<PhonemePlus>& vphoCorpus)
{
	...
	for (unsigned int iUnit = 0 ; iUnit < vuVector.size() ; iUnit++) {
		Segment segTemp;
		segTemp.dTimeBeg = static_cast<double> (vphoCorpus[ vuVector[iUnit].viPhonemePlus.front() ].iBegSec) + static_cast<double> (vphoCorpus[ vuVector[iUnit].viPhonemePlus.front() ].iBegMSec) / 10000000;
		segTemp.dTimeEnd = static_cast<double> (vphoCorpus[ vuVector[iUnit].viPhonemePlus.back() ].iEndSec) + static_cast<double> (vphoCorpus[ vuVector[iUnit].viPhonemePlus.back() ].iEndMSec) / 10000000;
...
//	vphoCorpus = fnReadCorpus("/net/yoyek64/dev/shmem/corpus.nlp", "/net/yoyek64/dev/shmem/corpus.lab", vcF0);
	...
	for (unsigned int iUnit = 0 ; iUnit < vuText.size() ; iUnit++) {
		...
		case -1:
			...
			uTemp.iType = -1;
			if (vphoText[vuText[iUnit].viPhonemePlus[0]].sName == "B") uTemp.viPhonemePlus.push_back(23);	// B
			else uTemp.viPhonemePlus.push_back(61);	// #	//AJ
		...
		case 0:
			...
			for (unsigned int iCorpus = 1 ; iCorpus < vphoCorpus.size() - 1 ; iCorpus++) {
				if (vphoText[ vuText[iUnit].viPhonemePlus[0] ].sName == vphoCorpus[iCorpus].sName) {
					...
					{
					 if (vphoText[ vuText[iUnit-1].viPhonemePlus.back() ].cCategory != vphoCorpus[iCorpus-1].cCategory && vphoText[ vuText[iUnit+1].viPhonemePlus.front() ].cCategory != vphoCorpus[iCorpus+1].cCategory) continue;
					}
					else if (iUnit > 0)
					{
						if (vphoText[ vuText[iUnit-1].viPhonemePlus.back() ].cCategory != vphoCorpus[iCorpus-1].cCategory) continue;
					}
					else
						if (vphoText[ vuText[iUnit+1].viPhonemePlus.front() ].cCategory != vphoCorpus[iCorpus+1].cCategory) continue;



					// eliminate phonemes with stress
					if (vphoCorpus[iCorpus].cStress == 1) continue;
			...
			if (vuTemp.empty()) {
				...
				cout << endl << "NUU module | error | unknown phoneme " << vphoText[ vuText[iUnit].viPhonemePlus[0] ].sName << " (no candidates found in corpus)";
				exit(1);
			}
			vCandidates.push_back(vuTemp);
			break;
		...
		case 1:
			...
			for (unsigned int iCorpus = 0 ; iCorpus < vphoCorpus.size() ; iCorpus++) {
				for (unsigned int iPhonemePlus = 0 ; vphoText[ vuText[iUnit].viPhonemePlus[iPhonemePlus] ].sName == vphoCorpus[iCorpus + iPhonemePlus].sName ; iPhonemePlus++) {
					if (iCorpus + iPhonemePlus + 1 == vphoCorpus.size()) break;	// end of corpus
					if (vphoCorpus[iCorpus + iPhonemePlus].cStress != 0) break; //stressed or weak //AJ					if (vphoCorpus[iCorpus].cSyllableFromWordEnd != vphoCorpus[iCorpus + iPhonemePlus].cSyllableFromWordEnd) break;
					if (iPhonemePlus+1 == vuText[iUnit].viPhonemePlus.size()) {
						// all phonemes in this unit have been checked (they match) - save candidate
						uTemp.clear();
						uTemp.iType = 1;
						for (unsigned int iTemp = 0 ; iTemp <= iPhonemePlus ; iTemp++) uTemp.viPhonemePlus.push_back(iCorpus + iTemp);
						uTemp.fTargetCost = fnUnitTargetCost(uTemp, vuText[iUnit], vphoCorpus, vphoText);
						vuTemp.push_back(uTemp);
			...
			if (!vuTemp.empty()) {
				...
				for (unsigned int iTemp = 0 ; iTemp < vuTemp.size() ; iTemp++) if (fMinTargetCostPerPhonemePlus > vuTemp[iTemp].fTargetCost / vuTemp[iTemp].viPhonemePlus.size()) fMinTargetCostPerPhonemePlus = vuTemp[iTemp].fTargetCost / vuTemp[iTemp].viPhonemePlus.size();
...
/*	// DISPLAY ALL UNITS WITH NUMBER OF CANDIDATES (vphoText i vCandidates)
	...
	for (unsigned int iUnit = 0 ; iUnit < vCandidates.size() - 4 ; ) {
		...
		*/
		...
		if (bInitialization) iWindowSize = 4;
		if (iUnit == vCandidates.size() - 5) iWindowSize = 5;
