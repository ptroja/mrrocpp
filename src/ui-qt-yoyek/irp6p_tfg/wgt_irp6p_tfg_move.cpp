...
void wgt_irp6p_tfg_move::on_pushButton_read_si_clicked()
{
	init_si();
}

int wgt_irp6p_tfg_move::init_si()
{
	...
	try {
		...
		if (robot.state.edp.pid != -1) {
			if (robot.state.edp.is_synchronised) // Czy robot jest zsynchronizowany?
			{
				...
				robot.ui_ecp_robot->read_joints(interface.irp6p_tfg->current_pos); // Odczyt polozenia walow silnikow
				ui.doubleSpinBox_cur_si->setValue(interface.irp6p_tfg->current_pos[0]);
...
void wgt_irp6p_tfg_move::on_pushButton_import_si_clicked()
{
	double val[robot.number_of_servos];
...
void wgt_irp6p_tfg_move::on_pushButton_export_si_clicked()
{
	std::stringstream buffer(std::stringstream::in | std::stringstream::out);

	buffer << "edp_irp6p_tfg JOINTS POSITION\n " << ui.doubleSpinBox_des_si->value();

	interface.ui_msg->message(buffer.str());
}

void wgt_irp6p_tfg_move::on_pushButton_copy_si_clicked()
{
	copy_si();
}

int wgt_irp6p_tfg_move::copy_si()
{
...
void wgt_irp6p_tfg_move::on_pushButton_execute_si_clicked()
{
	get_desired_position_si();
	move_it_si();
}

void wgt_irp6p_tfg_move::on_pushButton_l_si_clicked()
{
	get_desired_position_si();
...
void wgt_irp6p_tfg_move::on_pushButton_r_si_clicked()
{
	get_desired_position_si();
	robot.desired_pos[0] += ui.doubleSpinBox_step_si->value();
	move_it_si();
...
int wgt_irp6p_tfg_move::get_desired_position_si()
{
...
int wgt_irp6p_tfg_move::move_it_si()
{
	// wychwytania ew. bledow ECP::robot
	try {
