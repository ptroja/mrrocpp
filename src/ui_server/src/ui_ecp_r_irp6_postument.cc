...
// ---------------------------------------------------------------
ui_irp6_postument_robot::ui_irp6_postument_robot (edp_state_def* _edp_state, configurator &_config, sr_ecp* sr_ecp_msg)
	 : ecp_irp6_postument_robot (_config, sr_ecp_msg) { 
...
};// end: ui_irp6_postument_robot::ui_irp6_postument_robot ()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
/* // by Y - zdefiniowane w irp6_postument_robot - przemyslec czy nie trzeba wstawic warunku na poprawnosc synchronizacji
void ui_irp6_postument_robot::synchronise ( void ) { 
// Zlecenie synchronizacji robota
  ...
  if (EDP_command_and_reply_buffer.reply_package.reply_type == SYNCHRO_OK)
    synchronised = true;
};// end: ui_irp6_postument_robot::synchronise ()
*/
// ---------------------------------------------------------------
...
void ui_irp6_postument_robot::execute_motion (void) { 
...
}; // end: irp6_postument_robot::execute_motion (void)
// ---------------------------------------------------------------

// ---------------------------------------------------------------
void ui_irp6_postument_robot::set_desired_position ( double d_position[IRP6_POSTUMENT_NUM_OF_SERVOS] ) {
...
};// end: ui_irp6_postument_robot::set_desired_position()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
void ui_irp6_postument_robot::get_current_position ( double c_position[IRP6_POSTUMENT_NUM_OF_SERVOS]) {  
...
};// end: ui_irp6_postument_robot::get_current_position()
// ---------------------------------------------------------------
...
bool ui_irp6_postument_robot::get_kinematic (BYTE* kinematic_model_no)
{
...
bool ui_irp6_postument_robot::get_servo_algorithm ( BYTE algorithm_no[IRP6_POSTUMENT_NUM_OF_SERVOS],
      BYTE parameters_no[IRP6_POSTUMENT_NUM_OF_SERVOS])
{
 ...
 // Zlecenie odczytu numerow algorytmow i zestawow parametrow  
  ...
  memcpy (algorithm_no, EDP_command_and_reply_buffer.reply_package.rmodel.servo_algorithm.servo_algorithm_no,
	 IRP6_POSTUMENT_NUM_OF_SERVOS*sizeof(BYTE) );
...
bool ui_irp6_postument_robot::get_controller_state (controller_state_typedef* robot_controller_initial_state_l)
{
...
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::set_kinematic (BYTE kinematic_model_no)   { 
...
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::set_servo_algorithm (BYTE algorithm_no[IRP6_POSTUMENT_NUM_OF_SERVOS],
                            BYTE parameters_no[IRP6_POSTUMENT_NUM_OF_SERVOS] )   { 
...
// ZADANIE NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::set_tool_xyz_angle_axis ( double tool_vector[6] )
{
...
// ZADANIE NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::set_tool_xyz_euler_zyz ( double tool_vector[6] )
{
...
// ODCZYT NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::read_tool_xyz_angle_axis ( double tool_vector[6] )
{
...
// ODCZYT NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::read_tool_xyz_euler_zyz ( double tool_vector[6] )
{
...
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::move_motors ( double final_position[IRP6_POSTUMENT_NUM_OF_SERVOS] ) { 
...
//   	printf("is synchronised przed read motors\n");
  ...
  //   printf("przyslowiowa p... mokra \n");
	...
	//   for (j = 0; j < IRP6_POSTUMENT_NUM_OF_SERVOS; j++) { // by Y ORYGINAL
    for (j = 0; j < IRP6_POSTUMENT_NUM_OF_SERVOS; j++) {
...
// printf("\n ilosc krokow: %d, po ilu komun: %d, odleglosc 1: %f\n",EDP_command_and_reply_buffer.instruction.motion_steps, EDP_command_and_reply_buffer.instruction.value_in_step_no, EDP_command_and_reply_buffer.instruction.arm.coordinate_def.arm_coordinates[1]);
  ...
  if (is_synchronised()) 
    for (j = 0; j < IRP6_POSTUMENT_NUM_OF_SERVOS; j++) // Przepisanie aktualnych polozen
      current_position[j] = EDP_command_and_reply_buffer.reply_package.arm.coordinate_def.arm_coordinates[j];

  return true;
};// end: ui_irp6_postument_robot::move_motors()
// ---------------------------------------------------------------


// ---------------------------------------------------------------
bool ui_irp6_postument_robot::move_joints (double final_position[IRP6_POSTUMENT_NUM_OF_SERVOS] ) { 
...
// cprintf("NOS=%u\n",EDP_command_and_reply_buffer.instruction.motion_steps); 
 ...
 execute_motion();
  ...
  return true;
};// end: ui_irp6_postument_robot::move_internal()
// ---------------------------------------------------------------


// ---------------------------------------------------------------
bool ui_irp6_postument_robot::move_xyz_euler_zyz ( double final_position[7] ) { 
...
// cprintf("eNOS=%u\n",EDP_command_and_reply_buffer.instruction.motion_steps); 
 ...
 execute_motion();
  ...
  return true;
};// end: ui_irp6_postument_robot::move_xyz_euler_zyz()
// ---------------------------------------------------------------


// ---------------------------------------------------------------
bool ui_irp6_postument_robot::move_xyz_angle_axis ( double final_position[7] )
 {
...
// ---------------------------------------------------------------
bool ui_irp6_postument_robot::read_motors ( double current_position[IRP6_POSTUMENT_NUM_OF_SERVOS] ) { 
...
// printf("koniec read motors\n");
  return true;
};// end: ui_irp6_postument_robot::read_motors()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
bool ui_irp6_postument_robot::read_joints ( double current_position[IRP6_POSTUMENT_NUM_OF_SERVOS] ) { 
// Zlecenie odczytu polozenia
  int j;
// Parametry zlecenia ruchu i odczytu polozenia
  EDP_command_and_reply_buffer.instruction.instruction_type = GET;
 ...
 execute_motion();
  ...
  for (j = 0; j < IRP6_POSTUMENT_NUM_OF_SERVOS; j++) // Przepisanie aktualnych polozen
    current_position[j] = EDP_command_and_reply_buffer.reply_package.arm.coordinate_def.arm_coordinates[j];
  return true;
};// end: ui_irp6_postument_robot::read_joints()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
bool ui_irp6_postument_robot::read_xyz_euler_zyz (double current_position[IRP6_POSTUMENT_NUM_OF_SERVOS]) { 
// Zlecenie odczytu polozenia
  int j;
// Parametry zlecenia ruchu i odczytu polozenia
  EDP_command_and_reply_buffer.instruction.get_type = ARM_DV;
...
execute_motion();
  ...
  return true;
};// end: ui_irp6_postument_robot::read_external()
// ---------------------------------------------------------------


bool ui_irp6_postument_robot::read_xyz_angle_axis (double current_position[IRP6_POSTUMENT_NUM_OF_SERVOS]) {// Pobranie aktualnego polozenia ramienia robota
