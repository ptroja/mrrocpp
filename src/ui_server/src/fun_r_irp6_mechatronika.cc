...
int irp6m_read_kinematic()
{
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.pid!=-1)
		{
			...
			{
				ui_robot.irp6_mechatronika->get_kinematic(&model_no);
...
int irp6m_read_servo_algorithm()
{
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.pid!=-1)
		{
			if ( ui_state.irp6_mechatronika.edp.is_synchronised )
			{
				ui_robot.irp6_mechatronika->get_servo_algorithm(servo_alg_no, servo_par_no);
...
int irp6m_read_post_euler()
{
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.pid!=-1)
		{
			if ( ui_state.irp6_mechatronika.edp.is_synchronised )
			{
				ui_robot.irp6_mechatronika->read_xyz_euler_zyz(v);
				replySend(new Message('F','D','A',7,v,NULL));
			}
		}
	}
	CATCH_SECTION_UI
...
int irp6m_read_joints()
{
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.pid!=-1)
		{
			if ( ui_state.irp6_mechatronika.edp.is_synchronised )
			{
				ui_robot.irp6_mechatronika->read_joints(v);
				replySend(new Message('F','E','A',IRP6_MECHATRONIKA_NUM_OF_SERVOS,v,NULL));
			}
		}
	}
	CATCH_SECTION_UI
...
int irp6m_read_motors()
{
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.pid!=-1)
		{
			if ( ui_state.irp6_mechatronika.edp.is_synchronised )
			{
				ui_robot.irp6_mechatronika->read_motors(v);
				replySend(new Message('F','F','A',IRP6_MECHATRONIKA_NUM_OF_SERVOS,v,NULL));
			}
		}
	}
	CATCH_SECTION_UI
...
int irp6m_read_tool_angle()
{
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.pid!=-1)
		{
			if ( ui_state.irp6_mechatronika.edp.is_synchronised )  // Czy robot jest zsynchronizowany?
			{
				ui_robot.irp6_mechatronika->read_tool_xyz_angle_axis(v); // Odczyt polozenia walow silnikow
...
int irp6m_read_tool_euler()
{
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.pid!=-1)
		{
			if ( ui_state.irp6_mechatronika.edp.is_synchronised )
			{
				ui_robot.irp6_mechatronika->read_tool_xyz_euler_zyz(v);
				replySend(new Message('F','I','A',6,v,NULL));
			}
		}
	}
	CATCH_SECTION_UI
	...
	try
	{
		if (ui_state.irp6_mechatronika.edp.state == 0)
		{
			...
			)
			{
				...
				ui_robot.irp6_mechatronika = new ui_common_robot(*config, *ui_msg.all_ecp, lib::ROBOT_IRP6_MECHATRONIKA);
...
reload_irp6m_configuration ()
{
	...
	if ((ui_state.irp6_mechatronika.is_active = config->value<int>("is_irp6_mechatronika_active")) == 1)
	{
		...
		case 0:
			...
			ui_state.irp6_mechatronika.edp.node_name = config->value<std::string> ("node_name", ui_state.irp6_mechatronika.edp.section_name);
