...
// ---------------------------------------------------------------
ui_irp6_on_track_robot::ui_irp6_on_track_robot (edp_state_def* _edp_state, configurator &_config, sr_ecp* sr_ecp_msg) 
	: ecp_irp6_on_track_robot (_config, sr_ecp_msg) { 
...
// Zlecenie synchronizacji robota
  ...
  if (EDP_command_and_reply_buffer.reply_package.reply_type == SYNCHRO_OK)
    synchronised = true;
};// end: ui_irp6_on_track_robot::synchronise ()
*/
// ---------------------------------------------------------------
...
void ui_irp6_on_track_robot::execute_motion (void) {
// Zlecenie wykonania ruchu przez robota jest to polecenie dla EDP
	 EDP_command_and_reply_buffer.send(EDP_fd);
	 if (EDP_command_and_reply_buffer.reply_package.reply_type != ACKNOWLEDGE) {
//		 printf("gggg: %d\n", EDP_command_and_reply_buffer.reply_package.reply_type);
		   EDP_command_and_reply_buffer.query(EDP_fd);
	      perror ("Error in EDP ot");
...
// ---------------------------------------------------------------
void ui_irp6_on_track_robot::set_desired_position ( double d_position[IRP6_ON_TRACK_NUM_OF_SERVOS] ) {
...
};// end: ui_irp6_on_track_robot::set_desired_position()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
void ui_irp6_on_track_robot::get_current_position ( double c_position[IRP6_ON_TRACK_NUM_OF_SERVOS]) {  
// Pobranie aktualnych polozen
  for (int j = 0; j < IRP6_ON_TRACK_NUM_OF_SERVOS; j++) 
   c_position[j] = current_position[j];

};// end: ui_irp6_on_track_robot::get_current_position()
// ---------------------------------------------------------------
...
bool ui_irp6_on_track_robot::get_kinematic (BYTE* kinematic_model_no)
{
...
bool ui_irp6_on_track_robot::get_servo_algorithm ( BYTE algorithm_no[IRP6_ON_TRACK_NUM_OF_SERVOS],
      BYTE parameters_no[IRP6_ON_TRACK_NUM_OF_SERVOS])
{
 ...
 // Zlecenie odczytu numerow algorytmow i zestawow parametrow  
  ...
  memcpy (algorithm_no, EDP_command_and_reply_buffer.reply_package.rmodel.servo_algorithm.servo_algorithm_no,IRP6_ON_TRACK_NUM_OF_SERVOS*sizeof(BYTE) );
  memcpy (parameters_no, EDP_command_and_reply_buffer.reply_package.rmodel.servo_algorithm.servo_parameters_no,IRP6_ON_TRACK_NUM_OF_SERVOS*sizeof(BYTE) );
...
// do odczytu stanu poczatkowego robota
  bool ui_irp6_on_track_robot::get_controller_state (controller_state_typedef* robot_controller_initial_state_l)
{
...
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::set_kinematic (BYTE kinematic_model_no)   { 
...
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::set_servo_algorithm (BYTE algorithm_no[IRP6_ON_TRACK_NUM_OF_SERVOS],
                            BYTE parameters_no[IRP6_ON_TRACK_NUM_OF_SERVOS] )   { 
 ...
 // Zlecenie zapisu numerow algorytmow i zestawow parametrow  
  ...
  memcpy (EDP_command_and_reply_buffer.instruction.rmodel.servo_algorithm.servo_algorithm_no, algorithm_no,
	 IRP6_ON_TRACK_NUM_OF_SERVOS*sizeof(BYTE) );
...
// ZADANIE NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::set_tool_xyz_angle_axis ( double tool_vector[6] )
{
...
// ZADANIE NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::set_tool_xyz_euler_zyz ( double tool_vector[6] )
{
...
// ODCZYT NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::read_tool_xyz_angle_axis ( double tool_vector[6] )
{
...
// ODCZYT NARZEDZIA
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::read_tool_xyz_euler_zyz ( double tool_vector[6] )
{
...
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::move_motors ( double final_position[IRP6_ON_TRACK_NUM_OF_SERVOS] ) { 
...
//   	printf("is synchronised za read motors\n");
...
// printf("\n ilosc krokow: %d, po ilu komun: %d, odleglosc 1: %f\n",EDP_command_and_reply_buffer.instruction.motion_steps, EDP_command_and_reply_buffer.instruction.value_in_step_no, EDP_command_and_reply_buffer.instruction.arm.coordinate_def.arm_coordinates[1]);
 ...
 execute_motion();
  ...
  if (is_synchronised()) 
    for (j = 0; j < IRP6_ON_TRACK_NUM_OF_SERVOS; j++) // Przepisanie aktualnych polozen
      current_position[j] = EDP_command_and_reply_buffer.reply_package.arm.coordinate_def.arm_coordinates[j];


  return true;
};// end: ui_irp6_on_track_robot::move_motors()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::move_joints (double final_position[IRP6_ON_TRACK_NUM_OF_SERVOS] ) { 
...
// Parametry zlecenia ruchu i odczytu polozenia
 ...
 execute_motion();
  ...
  return true;
};// end: ui_irp6_on_track_robot::move_internal()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::move_xyz_euler_zyz ( double final_position[7] ) { 
...
// cprintf("eNOS=%u\n",EDP_command_and_reply_buffer.instruction.motion_steps); 
  ...
  for (j = 0; j < 6; j++) 
  {
   ...
   EDP_command_and_reply_buffer.instruction.arm.coordinate_def.gripper_coordinate = final_position[6];
 ...
 execute_motion();
  ...
  current_position[6] = EDP_command_and_reply_buffer.reply_package.arm.coordinate_def.gripper_coordinate;
    
  return true;
};// end: ui_irp6_on_track_robot::move_xyz_euler_zyz()
// ---------------------------------------------------------------


bool ui_irp6_on_track_robot::move_xyz_angle_axis ( double final_position[7] ) 
{
...
// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::read_motors ( double current_position[IRP6_ON_TRACK_NUM_OF_SERVOS] ) { 
...
// printf("koniec read motors\n");
  return true;
};// end: ui_irp6_on_track_robot::read_motors()
// ---------------------------------------------------------------


// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::read_joints ( double current_position[IRP6_ON_TRACK_NUM_OF_SERVOS] ) { 
// Zlecenie odczytu polozenia
  int j;
// Parametry zlecenia ruchu i odczytu polozenia
  EDP_command_and_reply_buffer.instruction.instruction_type = GET;
...
execute_motion();
  ...
  for (j = 0; j < IRP6_ON_TRACK_NUM_OF_SERVOS; j++) // Przepisanie aktualnych polozen
    current_position[j] = EDP_command_and_reply_buffer.reply_package.arm.coordinate_def.arm_coordinates[j];
  return true;
};// end: ui_irp6_on_track_robot::read_joints()
// ---------------------------------------------------------------

// ---------------------------------------------------------------
bool ui_irp6_on_track_robot::read_xyz_euler_zyz (double current_position[IRP6_ON_TRACK_NUM_OF_SERVOS]) { 
// Zlecenie odczytu polozenia
  int j;
// Parametry zlecenia ruchu i odczytu polozenia
  EDP_command_and_reply_buffer.instruction.get_type = ARM_DV;
...
execute_motion();
  ...
  return true;
};// end: ui_irp6_on_track_robot::read_external()
// ---------------------------------------------------------------


bool ui_irp6_on_track_robot::read_xyz_angle_axis (double current_position[IRP6_ON_TRACK_NUM_OF_SERVOS]) 
{ 
...
return true;
};// end ui_irp6_on_track_robot::read_xyz_angle_axis
