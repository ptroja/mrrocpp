...
void mp_task_vis_pbeclsac::task_initialization(void)
{
	...
	sensor_m[SENSOR_CAMERA_SA] =
		new ecp_mp::sensor::vis (SENSOR_CAMERA_SA, "[vsp_vis]", *this); //change if SENSOR_CAMERA_SA used for nonnn recog (vsp_vis_pbeolsac)
...
void mp_task_vis_pbeclsac::main_task_algorithm(void)
{
	...
	generator::mp_vis_pbeclsac_generator eyegen(*this, 4);
	...
	set_next_ecps_state ((int) ecp_mp::task::ECP_GEN_TEACH_IN, 0, "../trj/irp6ot_vis_2.trj", 1, ROBOT_IRP6_ON_TRACK);
	run_extended_empty_generator_for_set_of_robots_and_wait_for_task_termination_message_of_another_set_of_robots
			(1, 1, ROBOT_IRP6_ON_TRACK, ROBOT_IRP6_ON_TRACK);
