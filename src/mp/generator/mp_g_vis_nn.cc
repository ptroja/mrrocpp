...
#include "lib/srlib.h"
#include "mp/mp.h"
#include "mp/generator/mp_g_vis_nn.h"
...
double x2g=-0.1; //x nibytoola x2g=-0.1;
...
//#define PRINT 1
    ...
    nn_eye::nn_eye(task::task& _mp_task, int step): generator (_mp_task), irp6ot_con(1), irp6p_con(1){
        step_no = step;
    };
...
bool nn_eye::first_step () {
  ...
  printf("EYE GEN NN\n");
	...
	//TOOL
	irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[0][0]=1; irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[1][0]=0;
	irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[2][0]=0; irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[0][3]=0;

	irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[0][1]=0; irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[1][1]=1;
	irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[2][1]=0; irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[1][3]=0;

	irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[0][2]=0; irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[1][2]=0;
	...
	irp6ot->mp_command.instruction.set_arm_type = lib::XYZ_EULER_ZYZ; //POSE_FORCE_TORQUE_AT_FRAME;
...
bool nn_eye::next_step () {
 ...
 // UWAGA: dzialamy na jednoelementowej liscie robotow
	...
	if(node_counter!=1 && node_counter!=2)
	{
		...
		irp6ot->mp_command.instruction.instruction_type = lib::SET;
...
//frame1[1][0]=1; frame1[1][1]= 0; frame1[1][2]=0; frame1[1][3]= 11.0837;
//frame1[0][0]=0; frame1[0][1]= 1; frame1[0][2]= 0; frame1[0][3]= 75.2511;
//frame1[2][0]=0; frame1[2][1]=0; frame1[2][2]= 1; frame1[2][3]= 982.285;
...
//gdy zaslania chwytak
	...
	if(node_counter!=3 && node_counter!=4)
	{
		...
		measure[0][0]=frame1[1][3]; //-0.10
		measure[1][0]=frame1[0][3];//-0.024+0.004+0.017;
...
//	if(steps2switch>15) // to approach 15  //zaciskanie
//		vis_phase=1;
	...
	*/
	...
	/*
	if(alfa>M_PI/2 && alfa<=3*M_PI/4)
	{
		alfa-=M_PI/2;
		alfa*=-1;
	}
	else if(alfa>3*M_PI/4 && alfa<=M_PI)
	{
		alfa-=3*M_PI/4;
		alfa=M_PI/4-alfa;
	}
...
#ifdef PRINT
	cout <<"alfa="<<180*alfa/M_PI<<"     " <<alfa<<" alfa dobra =" << 180*atan2(frame1[0][0],frame1[1][0])/M_PI << endl;
	cout <<"beta="<<180*beta/M_PI<<"     " <<beta<<endl;
	cout <<"gamma="<<180*gammax/M_PI<<"     " <<gammax<<endl;
#endif
