...
#include "lib/srlib.h"
#include "mp/mp.h"
#include "mp/generator/mp_g_vis_pbeclsac.h"
...
#define PRINTB 1
    ...
    vis_pbeclsac::vis_pbeclsac(task::task& _mp_task, int step): generator (_mp_task), irp6ot_con(1), irp6p_con(1){
        step_no = step;
    };
...
bool vis_pbeclsac::first_step () {
  ...
  printf("EYE GEN XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
	...
	irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[2][0]=0; irp6ot->mp_command.instruction.rmodel.tool_frame_def.tool_frame[0][0]=0;
 ...
 // UWAGA: dzialamy na jednoelementowej liscie robotow
	...
	if(node_counter==3 || node_counter==4)
	{
		for (i=0; i<6; i++)
		{
			...
			measure[i][0]=0; //irp6ot->ecp_reply_package.reply_package.arm.pf_def.arm_coordinates[i]; //GDY NIE MA INFO Z KAM
			measure[i][1]=0;  //irp6ot->ecp_reply_package.reply_package.arm.pf_def.arm_coordinates[i];
			measure[i][2]=0; //irp6ot->ecp_reply_package.reply_package.arm.pf_def.arm_coordinates[i];
