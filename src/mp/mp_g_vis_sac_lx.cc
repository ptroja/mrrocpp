...
//			measure[i][j]=0;
	...
	irp6ot->ecp_td.next_tool_frame_m[2][2]=1; irp6ot->ecp_td.next_tool_frame_m[2][3]=0.25; //0.25;
	...
	irp6ot->ecp_td.motion_type = PF_FIXED_FRAME_TO_XYZ_ANGLE_AXIS_ABSOLUTE_POSE; //PF_FIXED_FRAME_WITH_DESIRED_FORCE_OR_SPEED;
...
bool mp_vis_sac_lx_generator::next_step () {
 ...
 // UWAGA: dzialamy na jednoelementowej liscie robotow
	...
	if(node_counter==1 || node_counter==2 || node_counter==3 || node_counter==4)
	{
		...
		//O_Tx_E.set_frame_tab(irp6ot->ecp_td.current_arm_frame_m); // zarem
		O_Tx_E.get_xyz_angle_axis(O_r_E[0]);
...
//std::cout << O_Tx_G;
	...
	O_Tx_E.set_frame_tab(irp6ot->ecp_td.MPcurrent_predicted_arm_frame_m);
	O_Tx_E.get_xyz_angle_axis(O_r_E[0]);
...
for(int i=0;i<6;i++) {
irp6ot->ecp_td.MPtoECP_position_velocity[i] = O_r_Ep[0][i];
}
/*	
for(int i=0;i<6;i++) {		
