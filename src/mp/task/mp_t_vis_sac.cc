...
#include "lib/srlib.h"
#include "mp/mp.h"
#include "mp/task/mp_t_vis_sac.h"
#include "mp/generator/mp_g_vis_sac.h"
#include "mp/generator/mp_g_force.h"
#include "ecp_mp/sensor/ecp_mp_s_vis_sac.h"
...
vis_sac::vis_sac(lib::configurator &_config) :
	task(_config)
{
	// Powolanie czujnikow
	sensor_m[lib::SENSOR_FORCE_ON_TRACK] = new ecp_mp::sensor::schunk (lib::SENSOR_FORCE_ON_TRACK, "[vsp_force_irp6ot]", *this);

	sensor_m[lib::SENSOR_CAMERA_SA] = new ecp_mp::sensor::vis_sac (lib::SENSOR_CAMERA_SA, "[vsp_vis]", *this); //change if SENSOR_CAMERA_SA used for nonnn recog (vsp_vis_pbeolsac)
...
{
	generator::vis_sac eyegen(*this, 4);
