...
#define PRINTB 1
    ...
    mp_vis_sac_generator::mp_vis_sac_generator(mp_task& _mp_task, int step): mp_generator (_mp_task), irp6ot_con(1), irp6p_con(1){ 
        step_no = step;          
    };  
...
bool mp_vis_sac_generator::first_step () {
  ...
  printf("EYE GEN XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
	...
	return true;
}; // end: mp_vis_sac_generator::first_step()

// ----------------------------------------------------------------------------------------------
// -----------------------------------  metoda	next_step -----------------------------------
// ----------------------------------------------------------------------------------------------

bool mp_vis_sac_generator::next_step () {
 ...
 // UWAGA: dzialamy na jednoelementowej liscie robotow
	...
	for(int i=0; i<4; i++)
	{
		for(int j=0; j<4; j++)
		{
			frame1[i][j]=vsp_vis_sac->image.vis_sac.frame_E_T_G[4*i+j];
		}
	}
	
	for(int i=0; i<6; i++)
		vec1[i]=0.001*vsp_vis_sac->image.vis_sac.frame_E_r_G__f[i];
