...
uint8_t NL_regulator_8_sarkofag::compute_set_value(void)
{
	...
	// step_new_pulse = step_new*SARKOFAG_INC_PER_REVOLUTION/(2*M_PI); // ORIGINAL
	step_new_pulse = step_new * SARKOFAG_AXIS_7_INC_PER_REVOLUTION / (2 * M_PI);//*AXE_7_POSTUMENT_TO_TRACK_RATIO);
...
#define MAX_REG_CURRENT 15.0
	...
	if (master.step_counter > SARKOFAG_GRIPPER_SUM_OF_CURRENTS_NR_OF_ELEMENTS) {
	...
	current_index = ((++current_index) % SARKOFAG_GRIPPER_SUM_OF_CURRENTS_NR_OF_ELEMENTS);
	...
	switch (reg_state)
	{
		case lib::GRIPPER_START_STATE:
			...
			if (sum_of_currents > SARKOFAG_GRIPPER_SUM_OF_CURRENTS_MAX_VALUE) {
		...
		case lib::GRIPPER_BLOCKED_STATE:
			...
			if (((master.step_counter - gripper_blocked_start_time) > GRIPPER_BLOCKED_TIME_PERIOD)
					&& (!(sum_of_currents > SARKOFAG_GRIPPER_SUM_OF_CURRENTS_MAX_VALUE * 100))) {
