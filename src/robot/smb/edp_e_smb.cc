...
void effector::get_arm_position(bool read_hardware, lib::c_buffer &instruction)
{
	...
	if (!robot_test_mode) {
		...
		for (int i = 0; i < lib::smb::LEG_CLAMP_NUMBER; i++) {
			edp_ecp_rbuffer.multi_leg_reply.leg[i].is_down = fai->is_lower_halotron_active(i);
			edp_ecp_rbuffer.multi_leg_reply.leg[i].is_up = fai->is_upper_halotron_active(i);
