...
robot::robot(const lib::robot_name_t & _robot_name, lib::configurator &_config, lib::sr_ecp &_sr_ecp) :
		...
		epos_disable_brake_command_data_port(lib::epos::EPOS_DISABLE_BRAKE_COMMAND_DATA_PORT, port_manager) ,
...
void robot::create_command()
{
	...
	if (epos_motor_command_data_port.get() == mrrocpp::lib::single_thread_port_interface::NewData) {
		...
		ecp_command.spkm.variant = lib::spkm::CBUFFER_VARIANT::POSE;

		ecp_command.spkm.set_pose_specification = lib::spkm::POSE_SPECIFICATION::MOTOR;
	...
	if (epos_joint_command_data_port.get() == mrrocpp::lib::single_thread_port_interface::NewData) {
		...
		ecp_command.spkm.set_pose_specification = lib::spkm::POSE_SPECIFICATION::JOINT;
	...
	if (epos_quickstop_command_data_port.get() == mrrocpp::lib::single_thread_port_interface::NewData) {
		...
		ecp_command.spkm.variant = lib::spkm::CBUFFER_VARIANT::QUICKSTOP;
	...
	if (epos_brake_command_data_port.get() == mrrocpp::lib::single_thread_port_interface::NewData) {
		...
		ecp_command.spkm.variant = lib::spkm::CBUFFER_VARIANT::BRAKE;

		check_then_set_command_flag(is_new_data);
	}

	if (epos_disable_brake_command_data_port.get() == mrrocpp::lib::single_thread_port_interface::NewData) {
		ecp_command.set_type = ARM_DEFINITION;
		if (!is_synchronised()) {
			ecp_command.motion_type = lib::RELATIVE;
			ecp_command.set_arm_type = lib::MOTOR;
		...
		ecp_command.spkm.variant = lib::spkm::CBUFFER_VARIANT::DISABLE_BRAKE;
	...
	if (epos_clear_fault_data_port.get() == mrrocpp::lib::single_thread_port_interface::NewData) {
		...
		ecp_command.spkm.variant = lib::spkm::CBUFFER_VARIANT::CLEAR_FAULT;

		check_then_set_command_flag(is_new_data);
	}

	if (epos_motor_reply_data_request_port.is_new_request()) {
		ecp_command.spkm.get_pose_specification = lib::spkm::POSE_SPECIFICATION::MOTOR;
	...
	if (epos_joint_reply_data_request_port.is_new_request()) {
		ecp_command.spkm.get_pose_specification = lib::spkm::POSE_SPECIFICATION::JOINT;
