...
int PtzProxy::SetCam(double pan, double tilt, double zoom)
{
  ...
  cmd.pan = htons((short)rint(100.0*RTOD(pan)));
  cmd.tilt = htons((short)rint(100.0*RTOD(tilt)));
  cmd.zoom = htons((short)rint(100.0*RTOD(zoom)));
...
int PtzProxy::SetSpeed(double panspeed, double tiltspeed)
{
  ...
  memset(&cmd,0,sizeof(cmd));
  cmd.panspeed = htons((short)rint(100.0*RTOD(panspeed)));
  cmd.tiltspeed = htons((short)rint(100.0*RTOD(tiltspeed)));
...
void PtzProxy::FillData(player_msghdr_t hdr, const char* buffer)
{
  ...
  pan = DTOR(((short)ntohs(((player_ptz_data_t*)buffer)->pan))/100.0);
  tilt = DTOR(((short)ntohs(((player_ptz_data_t*)buffer)->tilt))/100.0);
  zoom = DTOR(((short)ntohs(((player_ptz_data_t*)buffer)->zoom))/100.0);
  panspeed = DTOR(((short)ntohs(((player_ptz_data_t*)buffer)->panspeed))/100.0);
  tiltspeed = DTOR(((short)ntohs(((player_ptz_data_t*)buffer)->tiltspeed))/100.0);
}
