...
void FiducialProxy::FillData(player_msghdr_t hdr, const char* buffer)
{
  ...
  for(unsigned short i = 0; i < count && i < PLAYER_FIDUCIAL_MAX_SAMPLES; i++)
  {
    ...
    beacons[i].type = ntohs(data->fiducials[i].type);
...
if (beacons[i].type!=0){
    beacons[i].begin_index=ntohs(data->fiducials[i].begin_index);
    beacons[i].end_index=ntohs(data->fiducials[i].end_index);
    beacons[i].begin_x=((int32_t)ntohl(data->fiducials[i].begin_x)) / 1e3;
    beacons[i].end_x=((int32_t)ntohl(data->fiducials[i].end_x)) / 1e3;
    beacons[i].begin_y=((int32_t)ntohl(data->fiducials[i].begin_y)) / 1e3;
    beacons[i].end_y=((int32_t)ntohl(data->fiducials[i].end_y)) / 1e3;
    beacons[i].extra[0] = ((int32_t)ntohl(data->fiducials[i].extra[0])) / 1e3;
    beacons[i].extra[1] = ((int32_t)ntohl(data->fiducials[i].extra[1])) / 1e3;
    beacons[i].extra[2] = ((int32_t)ntohl(data->fiducials[i].extra[2])) / 1e3;
    beacons[i].extra[3] = ((int32_t)ntohl(data->fiducials[i].extra[3])) / 1e3;
    };
  }
}
