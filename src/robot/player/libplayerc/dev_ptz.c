...
void playerc_ptz_putdata(playerc_ptz_t *device, player_msghdr_t *header,
                         player_ptz_data_t *data, size_t len)
{
  device->pan = ((short) ntohs(data->pan)) * M_PI / 18000;
  device->tilt = ((short) ntohs(data->tilt)) * M_PI / 18000;
  device->zoom = ((short) ntohs(data->zoom)) * M_PI / 18000;
  return;
}
...
int playerc_ptz_set(playerc_ptz_t *device, double pan, double tilt, double zoom)
{
  ...
  cmd.pan = htons((uint16_t) (int16_t) (pan * 18000 / M_PI));
  cmd.tilt = htons((uint16_t) (int16_t) (tilt * 18000 / M_PI));
  cmd.zoom = htons((uint16_t) (int16_t) (zoom * 18000 / M_PI));
...
int playerc_ptz_set_ws(playerc_ptz_t *device, double pan, double tilt, double zoom, double panspeed, double tiltspeed)
{
  ...
  cmd.tilt = htons((uint16_t) (int16_t) (tilt * 18000 / M_PI));
  cmd.zoom = htons((uint16_t) (int16_t) (zoom * 18000 / M_PI));
  cmd.panspeed = htons((uint16_t) (int16_t) (panspeed * 18000 / M_PI));
  cmd.tiltspeed = htons((uint16_t) (int16_t) (tiltspeed * 18000 / M_PI));
