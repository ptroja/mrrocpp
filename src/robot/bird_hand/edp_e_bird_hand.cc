...
effector::effector(common::shell &_shell) :
	manip_effector(_shell, lib::bird_hand::ROBOT_NAME), macrostep_end_time(0),
			query_time(0)
{
...
void effector::move_arm(const lib::c_buffer &instruction)
{
	...
	if (!robot_test_mode)
	{
		...
		get_current_kinematic_model()->i2mp_transform(
				desired_motor_pos_new_tmp_rel, desired_joints_tmp_rel);
		dynamic_cast<kinematics::bird_hand::kinematic_model_bird_hand*> (get_current_kinematic_model())->i2mp_transform_synch(
				desired_motor_pos_new_tmp_abs, desired_joints_tmp_abs);

		for (unsigned int i = 0; i < number_of_servos; i++)
		{
			int16_t
					desired_torque_with_offset =
							(int16_t) ecp_edp_cbuffer.command_structure.finger[i].desired_torque
									+ torque_offset[i];
			...
			case mrrocpp::lib::bird_hand::SIGLE_STEP_POSTION_INCREMENT:
				device.setCMD1(
						(uint16_t) i,
						(int16_t) ecp_edp_cbuffer.command_structure.motion_steps,
						(int16_t) ecp_edp_cbuffer.command_structure.finger[i].reciprocal_of_damping,
						desired_torque_with_offset,
						(int32_t) desired_motor_pos_new_tmp_rel[i]);
				break;
			...
			case mrrocpp::lib::bird_hand::MACROSTEP_ABSOLUTE_POSITION:
				device.setCMD3(
						...
						(int16_t) ecp_edp_cbuffer.command_structure.finger[i].reciprocal_of_damping,
						desired_torque_with_offset,
						(int32_t) desired_motor_pos_new_tmp_abs[i]
...
void effector::get_arm_position(bool read_hardware, lib::c_buffer &instruction)
{
	...
	int err = clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &query_timespec,
			NULL);
	if (err != 0)
	{
	...
	else
	{
		for (uint8_t i = 0; i < number_of_servos; i++)
		{
			...
			desired_motor_pos_new_tmp[i] = (double) pos
					- synchro_position_motor[i];
			edp_ecp_rbuffer.status_reply_structure.finger[i].measured_current
					= c;
			edp_ecp_rbuffer.status_reply_structure.finger[i].meassured_torque
					= t - torque_offset[i];

			edp_ecp_rbuffer.status_reply_structure.finger[i].upper_limit_of_absolute_value_of_meassured_torque
					= status & (1 << TORQUE_LIMIT);
			edp_ecp_rbuffer.status_reply_structure.finger[i].lower_limit_of_absolute_position
					= status & (1 << LOWER_LIMIT);
			edp_ecp_rbuffer.status_reply_structure.finger[i].upper_limit_of_absolute_position
					= status & (1 << UPPER_LIMIT);
			edp_ecp_rbuffer.status_reply_structure.finger[i].upper_limit_of_measured_current
					= status & (1 << CURRENT_LIMIT);
		}

		dynamic_cast<kinematics::bird_hand::kinematic_model_bird_hand*> (get_current_kinematic_model())->mp2i_transform_synch(
	...
	if (!robot_test_mode)
	{
		...
		for (uint8_t j = 0; j < number_of_servos; j++)
		{
			...
			edp_ecp_rbuffer.configuration_reply_structure.finger[j].i_factor
					= i;
			edp_ecp_rbuffer.configuration_reply_structure.finger[j].d_factor
					= d;
...
{
	rb_obj
			= (boost::shared_ptr<common::reader_buffer>) new common::reader_buffer(
					*this);
	vis_obj = (boost::shared_ptr<common::vis_server>) new common::vis_server(
			*this);
}
