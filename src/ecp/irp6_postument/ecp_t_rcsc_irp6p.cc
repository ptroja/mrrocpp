...
// KONSTRUKTORY
ecp_task_rcsc_irp6p::ecp_task_rcsc_irp6p() : ecp_task()
{
	gt = NULL;
	nrg = NULL;
	rgg = NULL;
...
ecp_task_rcsc_irp6p::~ecp_task_rcsc_irp6p(){};
...
void ecp_task_rcsc_irp6p::main_task_algorithm(void)
{
	...
	sr_ecp_msg->message("ECP rcsc irp6p  - wcisnij start");
	ecp_wait_for_start();
		...
		for(;;) {
			...
			switch ( (RCSC_ECP_STATES) mp_command.mp_package.mp_2_ecp_next_state)
			{
				...
				case ECP_GEN_TFF_RUBIK_GRAB:
					switch ( (RCSC_RUBIK_GRAB_PHASES) mp_command.mp_package.mp_2_ecp_next_state_variant)
					{
				...
				case ECP_GEN_TFF_RUBIK_FACE_ROTATE:
					switch ( (RCSC_TURN_ANGLES) mp_command.mp_package.mp_2_ecp_next_state_variant)
					{
						case RCSC_CCL_90:
							rfrg->configure(-90.0);
						break;
...
{
	return new ecp_task_rcsc_irp6p();
};
