...
void ecp_irp6_postument_robot::get_arm_reply(void)
{
	...
	EDP_data.current_gripper_coordinate = EDP_command_and_reply_buffer.reply_package.arm.pf_def.gripper_coordinate;
	EDP_data.gripper_reg_state = EDP_command_and_reply_buffer.reply_package.arm.pf_def.gripper_reg_state;
}
...
void ecp_irp6_postument_robot::get_rmodel_reply(void)
{
	switch (EDP_command_and_reply_buffer.reply_package.rmodel_type) {
		...
		case FORCE_TOOL:
			...
			EDP_data.current_force_tool_weight = EDP_command_and_reply_buffer.reply_package.rmodel.force_tool.weight;
			break;
