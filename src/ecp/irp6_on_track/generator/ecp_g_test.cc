...
//				else if (i==3 && j==2) the_robot->ecp_command.instruction.rmodel.tool_frame_def.tool_frame[i][j]= 0.08;
	...
	return true;
}; // end: bool y_simple_generator::first_step (std::map <lib::SENSOR_t, lib::sensor*>& sensor_m, robot& the_robot )
// --------------------------------------------------------------------------


// --------------------------------------------------------------------------
bool y_simple::next_step (std::map <lib::SENSOR_t, lib::sensor*>& sensor_m ) {
