...
ecp_irp6_on_track_robot::ecp_irp6_on_track_robot(configurator &_config,
		sr_ecp *_sr_ecp) :
...
void ecp_irp6_on_track_robot::create_command(void)
{
	...
	EDP_command_and_reply_buffer.instruction.instruction_type
			= EDP_data.instruction_type;
	...
	EDP_command_and_reply_buffer.instruction.set_rmodel_type
			= EDP_data.set_rmodel_type;
	EDP_command_and_reply_buffer.instruction.get_rmodel_type
			= EDP_data.get_rmodel_type;
	EDP_command_and_reply_buffer.instruction.set_arm_type
			= EDP_data.set_arm_type;
	EDP_command_and_reply_buffer.instruction.get_arm_type
			= EDP_data.get_arm_type;
	EDP_command_and_reply_buffer.instruction.output_values
			= EDP_data.output_values;
	...
	case SET_GET:
		...
		if (EDP_data.set_type & RMODEL_DV)
		{
			...
			case TOOL_FRAME:
				copy_frame(
						EDP_command_and_reply_buffer.instruction.rmodel.tool_frame_def.tool_frame,
						EDP_data.next_tool_frame);
				break;
		...
		{
			EDP_command_and_reply_buffer.instruction.motion_type
					= EDP_data.motion_type;
			EDP_command_and_reply_buffer.instruction.interpolation_type
					= EDP_data.next_interpolation_type;
			EDP_command_and_reply_buffer.instruction.motion_steps
					= EDP_data.motion_steps;
			EDP_command_and_reply_buffer.instruction.value_in_step_no
					= EDP_data.value_in_step_no;
			// Wypelniamy czesc zwiazana z polozeniem ramienia
			switch (EDP_data.set_arm_type)
			{
			case FRAME:
				copy_frame(
						EDP_command_and_reply_buffer.instruction.arm.pf_def.arm_frame,
						EDP_data.next_arm_frame);
				break;
			case XYZ_ANGLE_AXIS:
				for (int j=0; j<6; j++)
...
void ecp_irp6_on_track_robot::get_reply(void)
{
	...
	{
	case ERROR:
		EDP_data.error_no.error0
				= EDP_command_and_reply_buffer.reply_package.error_no.error0;
		EDP_data.error_no.error1
				= EDP_command_and_reply_buffer.reply_package.error_no.error1;
		break;
	case ACKNOWLEDGE:
		break;
	case SYNCHRO_OK:
...
{
	EDP_data.input_values
			= EDP_command_and_reply_buffer.reply_package.input_values;
	for (int i=0; i<8; i++)
	{
		EDP_data.analog_input[i]
				=EDP_command_and_reply_buffer.reply_package.analog_input[i];
	}
}
...
void ecp_irp6_on_track_robot::get_rmodel_reply(void)
{
	...
	case TOOL_FRAME:
		copy_frame(
				EDP_data.current_tool_frame,
				EDP_command_and_reply_buffer.reply_package.rmodel.tool_frame_def.tool_frame);
		break;
	case TOOL_XYZ_ANGLE_AXIS:
		for (int i=0; i<6; i++)
