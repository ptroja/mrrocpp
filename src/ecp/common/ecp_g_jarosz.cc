...
ecp_linear_generator::ecp_linear_generator (ecp_task& _ecp_task,
        trajectory_description tr_des, int mp_communication_mode_arg): ecp_delta_generator (_ecp_task)
{
...
bool ecp_linear_generator::next_step (  )
{
    ...
    case MOTOR:
        ...
        for (i=0; i<MAX_SERVOS_NR; i++)
        {
            the_robot->EDP_data.next_motor_arm_coordinates[i] =
                the_robot->EDP_data.current_motor_arm_coordinates[i]
                + node_counter*td.coordinate_delta[i]/td.interpolation_node_no;
        } // end:for
        break;
    ...
    case XYZ_EULER_ZYZ:
        ...
        the_robot->EDP_data.next_gripper_coordinate =
            ...
            + node_counter*td.coordinate_delta[6]/td.interpolation_node_no;
        break;
    ...
    case XYZ_ANGLE_AXIS:
        ...
        for (i=0; i<6; i++)
        {
            the_robot->EDP_data.next_XYZ_AA_arm_coordinates[i] =
                the_robot->EDP_data.current_XYZ_AA_arm_coordinates[i]
                + node_counter*td.coordinate_delta[i]/td.interpolation_node_no;
        } // end:for
        the_robot->EDP_data.next_gripper_coordinate =
            the_robot->EDP_data.current_gripper_coordinate
...
ecp_linear_parabolic_generator::ecp_linear_parabolic_generator (ecp_task& _ecp_task,
        trajectory_description tr_des,	const double *time_a, const double *time_b):
        ecp_delta_generator (_ecp_task)
{
    td = tr_des;
...
bool ecp_linear_parabolic_generator::next_step ()
{
    ...
    case MOTOR:
        ...
        for (i=0; i<MAX_SERVOS_NR; i++)
        {
            double s = calculate_s((double)node_counter/td.interpolation_node_no,
                                   ta[i], tb[i] );
            the_robot->EDP_data.next_motor_arm_coordinates[i] =
                the_robot->EDP_data.current_motor_arm_coordinates[i]
                + s*td.coordinate_delta[i];
...
ecp_cubic_generator::ecp_cubic_generator (ecp_task& _ecp_task,
        trajectory_description tr_des, double *vp, double *vk ):
...
bool ecp_cubic_generator::next_step (  )
{
    ...
    case MOTOR:
        ...
        for (i=0; i<MAX_SERVOS_NR; i++)
        {
            acc[i] = ( 2.0*A2[i] + 6.0*A3[i]*(node_counter) ) *
                     ( 1.0 / (STEP*the_robot->EDP_data.motion_steps*STEP*the_robot->EDP_data.motion_steps));

            if ( fabs(acc[i] ) > a_max_motor[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i, fabs(acc[i]), a_max_motor[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
            ...
            vel[i] = ( A1[i] + 2.0*A2[i]*(node_counter) + 3.0*A3[i]*(node_counter*node_counter) ) *
                     ( 1.0 / (STEP*the_robot->EDP_data.motion_steps));

            if ( fabs(vel[i] ) > v_max_motor[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i, fabs(vel[i]) , v_max_motor[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
            ...
            the_robot->EDP_data.next_motor_arm_coordinates[i] =
                A0[i] + A1[i]*(node_counter) + A2[i]*(node_counter*node_counter) + A3[i]*( node_counter*node_counter*node_counter);
        } // end:for
        break;
    ...
    case XYZ_EULER_ZYZ:
        ...
        {
            acc[i] = 2.0*A2[i] + 6.0*A3[i]*(node_counter);
            if ( fabs(acc[i] * (1.0/(STEP*STEP)) ) > a_max_zyz[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i, fabs(acc[i]), a_max_zyz[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
            ...
            vel[i] = A1[i] + 2.0*A2[i]*(node_counter) + 3.0*A3[i]*(node_counter*node_counter);
            if ( fabs(vel[i] * (1.0/STEP) ) > v_max_zyz[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i, fabs(vel[i]) , v_max_zyz[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
    ...
    case XYZ_ANGLE_AXIS:
        ...
        {
            acc[i] = 2.0*A2[i] + 6.0*A3[i]*(node_counter);
            if ( fabs(acc[i] * (1.0/(STEP*STEP)) ) > a_max_aa[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i, fabs(acc[i]), a_max_aa[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
            ...
            vel[i] = A1[i] + 2.0*A2[i]*(node_counter) + 3.0*A3[i]*(node_counter*node_counter);
            if ( fabs(vel[i] * (1.0/STEP) ) > v_max_aa[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i, fabs(vel[i]) , v_max_aa[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
...
ecp_quintic_generator::ecp_quintic_generator (ecp_task& _ecp_task,
        trajectory_description tr_des, double *vp, double *vk,	  double *ap, double *ak):
        ecp_polynomial_generator(_ecp_task)
{
    td = tr_des;
    int tf=td.interpolation_node_no;
    ...
    for (int i=0; i<MAX_SERVOS_NR; i++)
    {
        A1[i]=vp[i];
        A2[i]=(ap[i]) / 2.0 ;
        A3[i]=( (20.0*td.coordinate_delta[i]) -  ( ( (8.0*vk[i])   + (12.0*vp[i]) )*tf )  - ( (3.0*ap[i]) - (ak[i])*(tf*tf) ) )
              / ( 2.0*(tf*tf*tf) ) ;
        A4[i]=( (-30.0*td.coordinate_delta[i]) + ( ( (14.0*vk[i]) + (16.0*vp[i]) )*tf ) +( (3.0*ap[i]) - (2.0*ak[i])*(tf*tf) ) )
              / ( 2.0*(tf*tf*tf*tf) );
        A5[i]=( (12.0*td.coordinate_delta[i])  -  ( ( (6.0*vk[i])  +  (6.0*vp[i]) )*tf )  -  ( ap[i]          -     ak[i])*(tf*tf)  )
              / ( 2.0*(tf*tf*tf*tf*tf) );
    } // end:for
...
bool ecp_quintic_generator::next_step ()
{
    ...
    case MOTOR:
        ...
        for (int i=0; i<MAX_SERVOS_NR; i++)
        {
            ...
            if ( fabs(acc[i] * (1.0/(STEP*STEP)) ) > a_max_motor[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i, fabs(acc[i]), a_max_motor[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
            } // end : if
            vel[i] = A1[i] + 2*A2[i]*(node_counter) + 3.0*A3[i]*(node_counter*node_counter) + 4.0*A4[i]*(node_counter*node_counter*node_counter) +
                     5.0*A5[i]*(node_counter*node_counter*node_counter*node_counter);
            if ( fabs(vel[i] * (1.0/STEP) ) > v_max_motor[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i, fabs(vel[i]) , v_max_motor[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
            ...
            the_robot->EDP_data.next_motor_arm_coordinates[i] =
                A0[i] + A1[i]*(node_counter) + A2[i]*(node_counter*node_counter) + A3[i]*( node_counter*node_counter*node_counter)
                + A4[i]*( node_counter*node_counter*node_counter*node_counter)
                + A5[i]*( node_counter*node_counter*node_counter*node_counter*node_counter);
        } // end:for
        break;
    ...
    case JOINT:
        ...
        for (int i=0; i<MAX_SERVOS_NR; i++)
        {
            acc[i] = 2.0*A2[i] + 6.0*A3[i]*(node_counter) + 12.0*A4[i]*(node_counter*node_counter) +
                     20.0*A5[i]*(node_counter*node_counter*node_counter) ;
            if ( fabs(acc[i] * (1.0/(STEP*STEP)) ) > a_max_joint[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i, fabs(acc[i]), a_max_joint[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
            ...
            vel[i] = A1[i] + 2*A2[i]*(node_counter) + 3.0*A3[i]*(node_counter*node_counter) + 4.0*A4[i]*(node_counter*node_counter*node_counter) +
                     5.0*A5[i]*(node_counter*node_counter*node_counter*node_counter);
            if ( fabs(vel[i] * (1.0/STEP) ) > v_max_joint[i] )
            { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i, fabs(vel[i]) , v_max_joint[i]);
                sr_ecp_msg.message(messg);
                throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
...
ecp_parabolic_teach_in_generator::ecp_parabolic_teach_in_generator (ecp_task& _ecp_task,
...
bool ecp_parabolic_teach_in_generator::next_step (  )
{
    ...
    if ( first_interval )
    {
        ...
        case MOTOR:
            ...
            for (i=0; i <MAX_SERVOS_NR; i++)
            {
                a[i] = 4.0*(tip.coordinates[i] - the_robot->EDP_data.current_motor_arm_coordinates[i])/(number_of_intervals*INTERVAL*number_of_intervals*INTERVAL);
                if ( fabs(a[i]) > a_max_motor[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                    sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i+1, fabs(a[i]), a_max_motor[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
                }
                if ( 0.5*fabs(a[i] * tip.motion_time) > v_max_motor[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
        ...
        case JOINT:
            ...
            for (i=0; i <MAX_SERVOS_NR; i++)
            {
                a[i] = 4.0*(tip.coordinates[i] - the_robot->EDP_data.current_joint_arm_coordinates[i])/(number_of_intervals*INTERVAL*number_of_intervals*INTERVAL);
                if ( fabs(a[i]) > a_max_joint[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                    sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i+1, fabs(a[i]), a_max_joint[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
                }
                if ( 0.5*fabs(a[i] * tip.motion_time) > v_max_joint[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
        ...
        case XYZ_EULER_ZYZ:
            ...
            for (i=0; i < 7; i++)
            {
                ...
                if ( fabs(a[i]) > a_max_zyz[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                    sprintf(messg,"Acceleration in coordinate %d is %f, max. acc = %f",i+1, fabs(a[i]), a_max_zyz[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
                }
                if ( 0.5*fabs(a[i] * tip.motion_time) > v_max_zyz[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                    sprintf(messg,"Velocity in coordinate %d is %f, max. vel = %f",i+1, 0.5*a[i] * tip.motion_time , v_max_zyz[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
                }
            }
        ...
        case XYZ_ANGLE_AXIS:
            ...
            for (i=0; i < 7; i++)
            {
                if (i==6)
                {
                    a[i] = 4.0*(tip.coordinates[i] - the_robot->EDP_data.current_gripper_coordinate)/(number_of_intervals*INTERVAL*number_of_intervals*INTERVAL);
                }
                else
                {
                    a[i] = 4.0*(tip.coordinates[i] - the_robot->EDP_data.current_XYZ_AA_arm_coordinates[i])/(number_of_intervals*INTERVAL*number_of_intervals*INTERVAL);
                }

                if ( fabs(a[i]) > a_max_aa[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                    sprintf(messg,"Velocity in coordinate %d is %f, max. vel = %f",i+1,0.5* a[i] * tip.motion_time , v_max_aa[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_ACCELERATION_EXCEEDED);
                }
                if ( 0.5*fabs(a[i] * tip.motion_time) > v_max_aa[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                    sprintf(messg,"Velocity in coordinate %d is %f, max. vel = %f",i+1,0.5*a[i] * tip.motion_time , v_max_aa[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
    ...
    if (node_counter <= number_of_intervals)
    {
        ...
        case MOTOR:
            ...
            for (i = 0; i <MAX_SERVOS_NR; i++)
            {
                if (node_counter < half_number_of_intervals)
                    the_robot->EDP_data.next_motor_arm_coordinates[i] =
                        the_robot->EDP_data.current_motor_arm_coordinates[i] +
                        0.5* a[i]* node_counter*INTERVAL*node_counter*INTERVAL;
                else
                    the_robot->EDP_data.next_motor_arm_coordinates[i] =
                        tip.coordinates[i] - 0.5 * a[i] * (number_of_intervals*INTERVAL - node_counter*INTERVAL)*
        ...
        case XYZ_EULER_ZYZ:
            ...
            if (node_counter < half_number_of_intervals)
                the_robot->EDP_data.next_gripper_coordinate =
                    the_robot->EDP_data.current_gripper_coordinate +
                    0.5* a[6]* node_counter*INTERVAL*node_counter*INTERVAL;
            else
                the_robot->EDP_data.next_gripper_coordinate =
                    tip.coordinates[6] - 0.5 * a[6] * (number_of_intervals*INTERVAL - node_counter*INTERVAL)*
                    (number_of_intervals*INTERVAL - node_counter*INTERVAL);
...
bool ecp_calibration_generator::next_step (  )
{
    ...
    if ( first_interval )
    {
        ...
        case MOTOR:
            ...
            for (i=0; i <MAX_SERVOS_NR; i++)
            {
                ...
                if ( fabs(0.5* a[i] * tip.motion_time) > v_max_motor[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                    sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i+1,0.5* a[i] * tip.motion_time , v_max_motor[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
        ...
        case JOINT:
            ...
            for (i=0; i <MAX_SERVOS_NR; i++)
            {
                ...
                if ( fabs(0.5* a[i] * tip.motion_time) > v_max_joint[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                    sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i+1,0.5* a[i] * tip.motion_time , v_max_joint[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
        ...
        case XYZ_EULER_ZYZ:
            ...
            for (i=0; i < 7; i++)
            {
                ...
                {
                    a[i] = 4.0*(tip.coordinates[i] - the_robot->EDP_data.current_XYZ_ZYZ_arm_coordinates[i])/(number_of_intervals*INTERVAL*number_of_intervals*INTERVAL);
                }
                ...
                if ( fabs(0.5* a[i] * tip.motion_time) > v_max_zyz[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                    sprintf(messg,"Velocity in coordinate %d is %f, max. vel = %f",i+1,0.5* a[i] * tip.motion_time , v_max_zyz[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
        ...
        case XYZ_ANGLE_AXIS:
            ...
            for (i=0; i < 7; i++)
            {
                ...
                if ( fabs(0.5* a[i] * tip.motion_time) > v_max_aa[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                    sprintf(messg,"Velocity in coordinate %d is %f, max. vel = %f",i+1,0.5* a[i] * tip.motion_time , v_max_aa[i]);
                    sr_ecp_msg.message(messg);
                    throw ECP_error (NON_FATAL_ERROR, MAX_VELOCITY_EXCEEDED);
        ...
        case MOTOR:
            for (i = 0; i <MAX_SERVOS_NR; i++)
            {
                if (node_counter < half_number_of_intervals)
                    ...
                    the_robot->EDP_data.next_motor_arm_coordinates[i] =
                        the_robot->EDP_data.current_motor_arm_coordinates[i] +
                        a[i] * (node_counter*INTERVAL*number_of_intervals*INTERVAL
                                -0.5*node_counter*INTERVAL*node_counter*INTERVAL
        ...
        case XYZ_EULER_ZYZ:
            ...
            if (node_counter < half_number_of_intervals)
                the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[6] =
                    the_robot->EDP_data.current_XYZ_ZYZ_arm_coordinates[6] +
                    0.5* a[6]* node_counter*INTERVAL*node_counter*INTERVAL;
            else
                the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[6] =
                    the_robot->EDP_data.current_XYZ_ZYZ_arm_coordinates[6] +
                    a[6] * (node_counter*INTERVAL*number_of_intervals*INTERVAL
                            -0.5*node_counter*INTERVAL*node_counter*INTERVAL
                            -0.25*number_of_intervals*INTERVAL*number_of_intervals*INTERVAL);
    ...
    { // Zniwelowanie ewentualnego bledu miedzy efektem interpolacji a faktyczna pozycja nauczona - celem ruchu
...
ecp_cubic_spline_generator::ecp_cubic_spline_generator(ecp_task& _ecp_task,
        double interval = 0.02):
        ecp_spline_generator (_ecp_task)
{
    INTERVAL = interval; // Dlugosc okresu interpolacji w [sek]
    ...
    if (node_counter <= number_of_intervals)
    {
        ...
        case MOTOR:
            for (i = 0; i <MAX_SERVOS_NR; i++)
            {
                ...
                the_robot->EDP_data.next_motor_arm_coordinates[i] =
                    A0[i] +  A2[i]*(node_counter*node_counter) + A3[i]*( node_counter*node_counter*node_counter);
            } // end : for
            break;
            // ---------------------------------------------------------------------------------------------------------
        case JOINT:
...
ecp_smooth_cubic_spline_generator::ecp_smooth_cubic_spline_generator (ecp_task& _ecp_task,
        double *vp, double *vk, double interval = 0.02):
        ecp_spline_generator (_ecp_task)
{
    INTERVAL = interval; // Dlugosc okresu interpolacji w [sek]
    int i;
...
void ecp_smooth_cubic_spline_generator::Build_Coeff (double *tt, double *yy, int nn, double vvp, double vvk, double *aa)
{
    ...
    for (i=1;i<=nn-2;i++)
    { // petla algorytmu trojdiagonalnego
...
bool ecp_smooth_cubic_spline_generator::next_step()
{
    ...
    if ( build_coeff )
    {
        ...
        for (int i=0; i<MAX_SERVOS_NR; i++)
        { // DLA KAZDEJ WSPOLRZEDNEJ BUDUJEMY WSPOLCZYNNIKI METODA BUILD_COEFF
    ...
    if (node_counter <= t(j+1,1))
    {
        ...
        case MOTOR:
            ...
            for (i = 0; i <MAX_SERVOS_NR; i++)
            {
                ...
                vel[i] = ( (y(j+1,i) - y(j,i)) / (t(j+1,i) - t(j,i)) ) -
                         ((3.0*A*A - 1.0)/6.0 ) * (t(j+1,i) - t(j,i)) * a(j,i) +
                         ((3.0*B*B - 1.0)/6.0 ) * (t(j+1,i) - t(j,i)) * a(j+1,i) ;
                if ( fabs(vel[i] * (1.0/STEP) ) > v_max_motor[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego predkosci
                    sprintf(messg,"Velocity in axis %d is %f, max. vel = %f",i, fabs(vel[i]) , v_max_motor[i]);
                    sr_ecp_msg.message(messg);
                ...
                the_robot->EDP_data.next_motor_arm_coordinates[i] = A*y(j,i) + B*y(j+1,i) + C*a(j,i) +D*a(j+1,i);
        ...
        case JOINT:
            ...
            for (i = 0; i <MAX_SERVOS_NR; i++)
            {
                ...
                the_robot->EDP_data.next_joint_arm_coordinates[i] = A*y(j,i) + B*y(j+1,i) + C*a(j,i) +D*a(j+1,i) ;
        ...
        case XYZ_EULER_ZYZ:
            ...
            for (i = 0; i < 7; i++)
            {
                ...
                {
                    the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[i] = A*y(j,i) + B*y(j+1,i) + C*a(j,i) +D*a(j+1,i) ;
                }
            } // end:for
            break;
            // ---------------------------------------------------------------------------------------------------------
        case XYZ_ANGLE_AXIS:
            ...
            for (i = 0; i < 6; i++)
            {
                ...
                {
                    the_robot->EDP_data.next_XYZ_AA_arm_coordinates[i] = A*y(j,i) + B*y(j+1,i) + C*a(j,i) +D*a(j+1,i) ;
                }
            } // end:for
            break;
        default:
...
bool ecp_quintic_spline_generator::next_step()
{
    ...
    if (node_counter <= number_of_intervals)
    {
        ...
        case MOTOR:
            for (i = 0; i <MAX_SERVOS_NR; i++)
            {
                acc[i] = 6.0*A3[i]*(node_counter) + 12.0*A4[i]*(node_counter*node_counter) +
                         20.0*A5[i]*(node_counter*node_counter*node_counter) ;
                if ( fabs(acc[i] * (1.0/(STEP*STEP)) ) > a_max_motor[i] )
                { // Sprawdzenie przekroczenia dopuszczalnego przyspieszenia
                    sprintf(messg,"Acceleration in axis %d is %f, max. acc = %f",i, fabs(acc[i]), a_max_motor[i]);
                ...
                the_robot->EDP_data.next_motor_arm_coordinates[i] =
                    A0[i] + A3[i]*( node_counter*node_counter*node_counter)
                    + A4[i]*(node_counter*node_counter*node_counter*node_counter)
                    + A5[i]*(node_counter*node_counter*node_counter*node_counter*node_counter);
            } // end : for
            break;
...
bool ecp_elipsoid_generator::next_step (  )
{
    ...
    if (node_counter < number_of_intervals)
    {
        ...
        case XYZ_EULER_ZYZ:
            // trajektoria zadana
            // elipsa
            the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[0] = 0.7 *
                    sin(0.03*(node_counter*INTERVAL-50));
            the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[1] = 1.5 - 0.4 *
                    cos(0.03*(node_counter*INTERVAL-50));
            the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[2] = 1;
            the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[3] =
                M_PI/2. -3./360.*M_PI * (node_counter*INTERVAL-50);
...
void ecp_save_trajectory ( ecp_elipsoid_generator& the_generator, ecp_task& _ecp_task)
{
    ...
    if (MsgSend(_ecp_task.UI_fd, &ecp_to_ui_msg,  sizeof(ECP_message),  &ui_to_ecp_rep, sizeof(UI_reply)) < 0)
    {// by Y&W
        e = errno;
