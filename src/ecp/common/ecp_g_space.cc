...
// ----------------------------------------------------------------------------------------------
irp6ot_hermite_spline_generator::irp6ot_hermite_spline_generator (ecp_task& _ecp_task,
	 double interval,double ts ) : ecp_teach_in_generator (_ecp_task) 
{
	...
	for (i=0;i<MAX_SERVOS_NR;i++){				// setting the values of the default velocities
		v_def_motor[i]=90;
		v_def_joint[i]=50;
...
void irp6ot_hermite_spline_generator::fill_hermite_arrays(void){
   if (pose_list.empty()) throw ECP_error (NON_FATAL_ERROR, NON_COMPATIBLE_LISTS);
   int i,j;
   ...
   for (i=0;i<MAX_SERVOS_NR;i++) yi[0][i]=starting_pose.coordinates[i]; // Copy the values of the coordinates of the current robot pose from the virtual image of the robot to the first entry in the array yi
   ...
   { 
	   for (i=0;i<MAX_SERVOS_NR;i++) yi[j][i]=pose_list_iterator->coordinates[i];
   ...
   for (j=1;j<=npoints;j++){
	   ...
	   switch ( starting_pose.arm_type ) {
    	  ...
    	  for(i=0;i<MAX_SERVOS_NR;i++){
		    ...
		    if(dtime[i]<TS*0.1) dtime[i]=TS*0.1;
			if (time[j]<time[j-1]+dtime[i]) time[j]=time[j-1]+dtime[i];
			// printf("\n   %f",dtime[i]);
		  }
		  break;
	      case JOINT:
...
// Filling 'yiprim'   
for (j=1;j<npoints;j++) for (i=0;i<6;i++){
	      h1=time[j]-time[j-1];
		  h2=time[j+1]-time[j];
		  mi=h1/(h1+h2);
		  la=1-mi;
	      ...
	      if(j==1) yiprim[0][i]= 0; // 1.5 * dy1[j] - 0.5 * dy2[j];
          if(j==npoints-1) yiprim[npoints][i]= 0; // -0.5 * dy1[j] + 1.5 * dy2[j]; 
   }
...
bool irp6ot_hermite_spline_generator::next_step (  ) {
	...
	int i;					
	if (last_time) {   fclose(log_file); 	return false;}
	if ( first_time ) {
    
	// Fill the virtual image of the robot with data received from the EDP 
    the_robot->get_reply();

	// Copy the coordinates of the current robot pose from the virtual image of the robot to the structure 'starting_pose'
    switch ( starting_pose.arm_type ) {
	case MOTOR:
		for(i=0; i<MAX_SERVOS_NR; i++) starting_pose.coordinates[i]=the_robot->EDP_data.current_motor_arm_coordinates[i];
        // for(i=0; i<6; i++) printf("\n next step MOTOR %f",starting_pose.coordinates[i]);     
		break;
	case JOINT:
		for(i=0; i<MAX_SERVOS_NR; i++) starting_pose.coordinates[i]=the_robot->EDP_data.current_joint_arm_coordinates[i];
		break;
	case XYZ_EULER_ZYZ:
		for(i=0; i<6; i++) starting_pose.coordinates[i]=the_robot->EDP_data.current_XYZ_ZYZ_arm_coordinates[i];
		...
		break;
	case XYZ_ANGLE_AXIS:
		for(i=0; i<6; i++) starting_pose.coordinates[i]=the_robot->EDP_data.current_XYZ_AA_arm_coordinates[i];
		 starting_pose.coordinates[6]=the_robot->EDP_data.current_gripper_coordinate;
		break;
	default:
		 throw ECP_error (NON_FATAL_ERROR, INVALID_POSE_SPECIFICATION);
	...
	switch ( starting_pose.arm_type ) {
	case MOTOR:
		for (i = 0; i < MAX_SERVOS_NR; i++) the_robot->EDP_data.next_motor_arm_coordinates[i] = y[i];
	    break;
	case JOINT:
		for (i = 0; i < MAX_SERVOS_NR; i++) the_robot->EDP_data.next_joint_arm_coordinates[i] = y[i];
		break;
	case XYZ_EULER_ZYZ:
		for (i = 0; i < 6; i++) the_robot->EDP_data.next_XYZ_ZYZ_arm_coordinates[i] =y[i];
		the_robot->EDP_data.next_gripper_coordinate = y[6];
		break;
	case XYZ_ANGLE_AXIS:
		for (i = 0; i < 6; i++) the_robot->EDP_data.next_XYZ_AA_arm_coordinates[i] = y[i];
		the_robot->EDP_data.next_gripper_coordinate = y[6];
		break;
	default:
		throw ECP_error (NON_FATAL_ERROR, INVALID_POSE_SPECIFICATION);
    } // end: switch   
    the_robot->create_command (); // Creating command fro EDP
    return true;
...
// ----------------------------------------------------------------------------------------------
irp6ot_natural_spline_generator::irp6ot_natural_spline_generator(ecp_task& _ecp_task,
...
double dx1[MAX_SERVOS_NR],dx2[MAX_SERVOS_NR],dy1[MAX_SERVOS_NR],dy2[MAX_SERVOS_NR],h,p[MAX_SERVOS_NR],dtime[MAX_SERVOS_NR];
   ...
   if(npoints<3) throw ECP_error (NON_FATAL_ERROR, NON_COMPATIBLE_LISTS);
   ...
   for (i=0;i<MAX_SERVOS_NR;i++) yi[0][i]=starting_pose.coordinates[i];
   initiate_pose_list();  // reset list pointers
   j=1;
   while (pose_list_iterator != pose_list.end()) // Copy the values of the coordinates of the control poses from the list pose_list_ptr to the array yi
   { 
...
bool irp6ot_natural_spline_generator::first_step ()
{
    ...
    the_robot->create_command (); // Creating command for EDP
...
bool irp6ot_natural_spline_generator::next_step ()
{
	...
	if ( first_time ) {
	    ...
	    the_robot->get_reply();   // Fill the virtual image of the robot with data received from the EDP 
    ...
    the_robot->create_command (); // // Copy the data from the virtual image of the robot to the communication buffer sent to the EDP
return true;
