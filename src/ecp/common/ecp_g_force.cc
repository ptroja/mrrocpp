...
//		the_robot->EDP_data.force_axis_quantity=3; // DOBRZE
		...
		the_robot->EDP_data.motion_type = PF_MOVING_FRAME_WITH_DESIRED_FORCE_OR_SPEED;
...
bool y_edge_follow_force_generator::next_step ( )
{
	...
	for (int i=0; i<6;i++) inc_delta[i] = -inc_delta[i];
	...
	for (int i=0; i<6;i++) inc_delta[i]+=tmp_delta[i];
...
y_drawing_teach_in_force_generator::y_drawing_teach_in_force_generator(ecp_task& _ecp_task, int step): 
	ecp_teach_in_generator (_ecp_task) 			{	step_no=step;	};
...
//		the_robot->EDP_data.selection_vector[0] = FORCE_SV_AX;
//		the_robot->EDP_data.ECPtoEDP_force_xyz_torque_xyz[0] = -7;
