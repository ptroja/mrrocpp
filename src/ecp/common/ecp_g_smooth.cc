...
void ecp_smooth_generator::calculate(void)
{
	...
	double tk=10*STEP; //czas jednego makrokroku
	  ...
	  for(i=0; i<MAX_SERVOS_NR; i++)
	  {
		...
		// kierunek ruchu
		if(final_position[i]-start_position[i] < 0){ k[i]=-1;}
			else {k[i]=1;}
	  ...
	  for(i=0; i<MAX_SERVOS_NR; i++) if(s[i]==0) v_r[i]=0; else
	  {//pierszy model ruchu - przyspieszanie / hamowanie
		...
		if((v_1>=0)&&(v_1<=v_r[i])&&(v_1>=v_p[i])&&(v_1>=v_k[i])) v_r[i]=v_1;
		else if((v_2>=0)&&(v_2<=v_r[i])&&(v_2>=v_p[i])&&(v_2>=v_k[i])) v_r[i]=v_2;
		else
		{//drugi model ruchu - przyspieszanie / przyspieszanie
	  ...
	  for(i=0;i<MAX_SERVOS_NR;i++) td.coordinate_delta[i] = final_position[i]-
          start_position[i];
		if(debug)
		{          
...
if(v_grip<v_grip_min) v_grip=v_grip_min;
...
bool ecp_smooth_generator::next_step ()
{
 ...
 if ( first_interval )
 {
	...
	{
		case MOTOR:
			for(i=0;i<MAX_SERVOS_NR;i++) start_position[i]=the_robot->EDP_data.current_motor_arm_coordinates[i];
			for(i=0; i<MAX_SERVOS_NR; i++)
		 	{
        ...
        case JOINT:
	        	for(i=0;i<MAX_SERVOS_NR;i++) start_position[i]=the_robot->EDP_data.current_joint_arm_coordinates[i];
			for(i=0; i<MAX_SERVOS_NR; i++)
		 	{
	    ...
	    case XYZ_EULER_ZYZ:
			for(i=0;i<6;i++) start_position[i]=the_robot->EDP_data.current_XYZ_ZYZ_arm_coordinates[i];
		  ...
		  break;
        case XYZ_ANGLE_AXIS:
	        for(i=0;i<6;i++) start_position[i]=the_robot->EDP_data.current_XYZ_AA_arm_coordinates[i];
			start_position[6]=the_robot->EDP_data.current_gripper_coordinate;
			start_position[7]=0.0;
			for(i=0; i<MAX_SERVOS_NR; i++)
		 	{
   	...
   	switch ( td.arm_type ) {
		...
		if(node_counter < td.interpolation_node_no)
		{
			...
			//PROBA Z CHWYTAKIEM
		     ...
		     if(the_robot->robot_name == ROBOT_IRP6_ON_TRACK) i=8;
			else if(the_robot->robot_name == ROBOT_IRP6_POSTUMENT) i=7;
