...
void ecp_speaker_robot::create_command (void) {
  ...
  switch (EDP_data.instruction_type) {
    case SET:
    case SET_GET:
    	strcpy(EDP_buffer.instruction.arm.text_def.text, EDP_data.text);
	strcpy(EDP_buffer.instruction.arm.text_def.prosody, EDP_data.prosody);
     break;
    case GET:
    case QUERY:
         break;
...
EDP_data.reply_type = EDP_buffer.reply_package.reply_type;
 ...
 switch (EDP_data.reply_type) {
   case ERROR:
     ...
     break;
   case  ACKNOWLEDGE:
       EDP_data.speaking = EDP_buffer.reply_package.arm.text_def.speaking;
   break;
