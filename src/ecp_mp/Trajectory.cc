...
Trajectory::Trajectory(const Trajectory &trajectory)
{
	...
	trjPoses = new std::list<ecp_mp::common::smooth_taught_in_pose>(trajectory.trjPoses->begin(), trajectory.trjPoses->end());
}
...
{
	actPose = new ecp_mp::common::smooth_taught_in_pose();
...
{
	return new std::list<ecp_mp::common::smooth_taught_in_pose>(*trjPoses);
}
