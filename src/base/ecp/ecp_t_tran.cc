	...
	task(_config)
{
	...
	} else if (config.section_name == lib::irp6p_tfg::ECP_IRP6P_TFG_SECTION) {
	...
	} else if (config.section_name == lib::speaker::ECP_SPEAKER_SECTION) {
		ecp_m_robot = new speaker::robot(*this);
	} else if (config.section_name == lib::irp6m::ECP_SECTION) {
		ecp_m_robot = new irp6m::robot(*this);
	} else if (config.section_name == lib::polycrank::ECP_POLYCRANK_SECTION) {
