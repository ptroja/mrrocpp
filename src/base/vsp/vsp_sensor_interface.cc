	...
	mrrocpp_network_path(config.return_mrrocpp_network_path())
{
	...
	sr_msg = (boost::shared_ptr<lib::sr_vsp>) new lib::sr_vsp(lib::VSP, config.value <std::string> ("resourceman_attach_point"), config.return_attach_point_name("sr_attach_point", lib::UI_SECTION));
