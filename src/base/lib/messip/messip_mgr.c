...
static void debug_show(void)
{
	...
	if (nb_connexions > 0) {
		...
		for (index = 0; index < nb_connexions; index++) {
			...
			printf("%3d:%8d %-12s " TID_DISPLAY " %-12s %5d %6d %-18s", index, cnx->pid, cnx->process_name, cnx->tid, inet_ntoa(cnx->xclient_addr.sin_addr), cnx->xclient_addr.sin_port, cnx->sockfd, when);
	...
	printf("\n%d active channels\n", nb_channels);
	if (nb_channels > 0) {
		printf("          Pid Process             Tid Address       Port Name            Socket Created            Buffered Clients\n");
		for (index = 0; index < nb_channels; index++) {
			int k;
			char tmp[100];
			...
			printf("%c%3d:%8d %-12s " TID_DISPLAY " %-12s %5d %-16s %5d %-18s %5d/%-5d %d%s\n", (ch->f_notify_deaths) ? 'D' : ' ', index, ch->pid, ch->channel_name, ch->tid, ch->sin_addr_str, ch->sin_port, ch->channel_name, ch->sockfd, when, ch->nb_msg_buffered, ch->maxnb_msg_buffered, ch->nb_clients, tmp);
		} // for
	} // if
	...
	if (nb_proxies > 0) {
		...
		for (index = 0; index < proxies_sz; index++) {
			...
			if (p->nbytes == -1) // Slot available ?
				continue;
			printf("%2d:%9d " TID_DISPLAY " %8d %5d %8d %7d\n", index, p->pid_from, p->tid_from, p->pid_to, p->nbytes, p->priority, p->nb_proxies_to_trigger);
		} // for (index)
	} // if
...
static void http_table_column_title(char *msg, const char *title, const char *tag)
{
...
static void http_table_column_add_tid(char **buff, pthread_t val)
{
	char tmp[100];
...
static void http_build_table_channels(char *msg)
{
	...
	for (index = 0; index < nb_channels; index++) {
		...
		index, ch->pid, ch->channel_name, ch->tid, ch->sin_addr_str, ch->sin_port, ch->channel_name, ch->sockfd, when, ch->nb_msg_buffered, ch->maxnb_msg_buffered, ch->nb_clients, tmp);
	} // for
...
static void http_build_table_proxies(char *msg)
{
	int index;
	...
	for (index = 0; index < proxies_sz; index++) {
		...
		sprintf(&msg[strlen(msg)], "    <tr>\n"
			...
			"      <td valign=\"top\" align=\"right\" >%d<br>\n"
			"      </td>\n"
			"    </tr>\n", index, p->pid_from, p->pid_to, p->nbytes, p->priority, p->nb_proxies_to_trigger);
	} // http_build_table_connexions
...
static int http_send_status(int sockfd, int version, int subversion, int key)
{
	...
	sprintf(msg2, "<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\">\n"
		"<html>\n"
		"<head>\n"
		...
		"</head>\n");
	strcat(msg2, "<b>MessIP</b> : Message Passing over TCP/IP<br>"
		"<a href=\"http://singla.us/messip/\">http://singla.us/messip/</a><br>");
	sprintf(&msg2[strlen(msg2)], "Version <b>%d.%d.%c</b> compiled on <b>%s %s</b><br>", VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH
			+ 'a' - 1, __DATE__, __TIME__);
	strcat(msg2, "<hr width=\"100%\" size=\"2\"><br>");

	/*--- Active connections ---*/
	sprintf(&msg2[strlen(msg2)], "<body text=\"#000000\" bgcolor=\"#dddddd\" link=\"#000099\" vlink=\"#990099\" alink=\"#000099\">\n"
	...
	/*--- Print out the defined proxies ---*/
	sprintf(&msg2[strlen(msg2)], "<p>%d (%d) defined proxies\n", nb_proxies, proxies_sz);
	if (nb_proxies > 0)
	...
	for (;;) {
		...
		if (dcount >= 15) {
			...
			if (status == 3) {
				...
				else if (!strcmp(tag, "/connexion/pid"))
					key = KEY_CONNEXION_PID;
				else if (!strcmp(tag, "/connexion/process"))
					key = KEY_CONNEXION_PROCESS;
				else if (!strcmp(tag, "/connexion/tid"))
					key = KEY_CONNEXION_TID;
...
http_thread(void *arg)
{
	...
	if (status == EADDRINUSE) {
		fprintf(stderr, "%s %d\n\tUnable to bind, port %d is already in use\n", __FILE__, __LINE__, port_http);
		if (close(sockfd) == -1)
			fprintf(stderr, "Error while closing socket %d: %s\n", sockfd, strerror(errno));
		exit(-1);
	}
	if (status < 0) {
		fprintf(stderr, "%s %d\n\tUnable to bind - port %d - status=%d: %s\n", __FILE__, __LINE__, port_http, status, strerror(errno));
		if (close(sockfd) == -1)
			fprintf(stderr, "Error while closing socket %d: %s\n", sockfd, strerror(errno));
		exit(-1);
	}
...
static int client_channel_create(int sockfd, struct sockaddr_in *client_addr, connexion_t ** cnx)
{
	...
	inet_ntoa(client_addr->sin_addr), client_addr->sin_port, msg.hostname, msg.channel_name);
#endif

	/*--- Is there any channel with this name ? ---*/
	memset(&reply, 0, sizeof(reply));
	LOCK;
	pch = (channel_t **) bsearch(msg.channel_name, channels, nb_channels, sizeof(channel_t *), bsearch_channels);
	if (pch != NULL) {
	...
	if (ch) {
		if (!reply.f_already_connected) {
			...
			if (ch->nb_clients == 0)
				...
				ch->cnx_clients = (int *) realloc(ch->cnx_clients, (ch->nb_clients + 1) * sizeof(int));
			...
			if (cnx->nb_cnx_channels == 0)
				...
				cnx->sockfd_cnx_channels = (int *) realloc(cnx->sockfd_cnx_channels, (cnx->nb_cnx_channels + 1)
						* sizeof(int));
	...
	for (;;) {
		...
		while (nb > 0) {
			...
			if (dcount != (ssize_t) (sizeof(datasend) + bmsg->datalen + sizeof(int32_t))) {
				printf("dcount=%zd expected=%zd datalen=%d\n", dcount, sizeof(datasend) + bmsg->datalen
						+ sizeof(int32_t), bmsg->datalen);
			}
			assert( dcount ==
			...
			if (--ch->nb_msg_buffered == 0) {
				...
				ch->buffered_msg = (buffered_msg_t **) realloc(ch->buffered_msg, sizeof(buffered_msg_t *)
						* ch->nb_msg_buffered);
			}
...
thread_client_trigger_proxy(void *arg)
{
	...
	for (;;) {
		...
		while (nb > 0) {
			...
			if (dcount != tlen)
				printf("dcount=%zd expected=%d nbytes=%d - %zd - data=%p: %s\n", dcount, tlen, proxy->nbytes, sizeof(datasend), proxy->data, strerror(errno));
			assert( dcount == tlen );
...
static int client_proxy_attach(int sockfd, struct sockaddr_in *client_addr)
{
	ssize_t dcount;
	channel_t *ch;
	connexion_t *cnx;
	...
	if (dcount != sizeof(messip_send_proxy_attach_t)) {
		fprintf(stderr, "%s %d\n\tread %zd of %zd: %s\n", __FILE__, __LINE__, dcount, sizeof(messip_send_proxy_attach_t), strerror(errno));
		return -1;
	}
	...
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
	pthread_create(&proxy->tid_client_proxies_to_trigger, &attr, thread_client_trigger_proxy, proxy);
	UNLOCK;
	...
	if (proxy->process_to_trigger_sockfd == -1) {
		...
		sockaddr.sin_port = htons(ch->sin_port);
		sockaddr.sin_addr.s_addr = ch->sin_addr;
		if (connect(proxy->process_to_trigger_sockfd, (const struct sockaddr *) &sockaddr, sizeof(sockaddr)) < 0) {
			UNLOCK;
			fprintf(stderr, "%s %d\n\tUnable to connect to host %s, port %d: %s\n", __FILE__, __LINE__, inet_ntoa(sockaddr.sin_addr), sockaddr.sin_port, strerror(errno));
...
static int client_proxy_detach(int sockfd, struct sockaddr_in *client_addr)
{
	ssize_t dcount;
	struct iovec iovec[1];
	...
	if (dcount != sizeof(messip_send_proxy_detach_t)) {
		fprintf(stderr, "%s %d\n\tread %zd of %zd: %s\n", __FILE__, __LINE__, dcount, sizeof(messip_send_proxy_detach_t), strerror(errno));
		return -1;
	}
...
static int client_proxy_trigger(int sockfd, struct sockaddr_in *client_addr)
{
	ssize_t dcount;
	proxy_t *proxy = NULL;
	struct iovec iovec[1];
...
static int client_proxy_get_owner(int sockfd, struct sockaddr_in *client_addr)
{
	ssize_t dcount;
	proxy_t *proxy = NULL;
	struct iovec iovec[1];
...
static int notify_server_death_client(channel_t * ch, pid_t pid, pthread_t tid, int code)
{
	messip_datasend_t datasend;
	int sockfd;
	struct iovec iovec[1];
...
int main(int argc, char *argv[])
{
	...
	if (status < 0) {
		fprintf(stderr, "%s %d\nUnable to bind - port %d - status=%d: %s\n", __FILE__, __LINE__, port, status, strerror(errno));
		if (close(sockfd) == -1)
			fprintf(stderr, "Error while closing socket %d: %s\n", sockfd, strerror(errno));
		return -1;
	}
